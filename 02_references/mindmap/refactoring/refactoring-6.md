...
- Procedure
	...
	- 변수 접근 범위 제한
	- 테스트
	- 변수 값이 레코드라면 _레코드 캡슐화하기 (7.1절)_ 을 적용할지 고려
...
- Procedure
	- 이름을 변경할 때 사용 범위도 고려해보자.<br/>폭 넓게 사용되는 변수라면 _변수 캡슐화 하기 (6.6절)__ 을 고려
...
- Procedure
	...
	- 테스트
	- _함수 선언 바꾸기 (6.5절)_로 새 데이터 구조로 매개변수를 활용
	- 테스트
...
- Procedure
	...
	-  공통 레코드를 사용하는 함수 각각을 새 클래스로 옮긴다.<br/>(_함수 옮기기 (8.1절)_)
	-  데이터를 조작하는 로직들은<br/>_함수로 추출 (6.1절)_ 해서 새 클래스로 옮긴다
