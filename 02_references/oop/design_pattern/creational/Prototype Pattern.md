...
>부품이 빠진 로봇을 앞에 두고, 옆에 이미 조립된 로봇 세 대가 서 있음. 처음부터 하나하나 조립하는 대신 이미 완성된 로봇을 그대로 복제하면 훨씬 빠름 — 완성된 로봇의 내부 구조(어떤 부품이 어떻게 조립됐는지)를 몰라도 복제 자체는 가능해야 한다는 게 Prototype 의 핵심.

![Untitled](../../../../_assets/oop/Untitled%2038.png)

>"자, 복제 좀 해볼까." Config A/B/C 처럼 설정이 조금씩 다른 원형(prototype)들을 미리 준비해두면, 이후로는 필요한 설정에 맞는 원형을 골라 복제만 하면 됨 — 설정별로 서브클래스를 따로 만들 필요가 없어짐.
