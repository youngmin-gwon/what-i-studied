...
다른 도메인에도 같은 구조가 쓰임. (아래 Structure 부터는 다시 도형 예시로 돌아감.)
...
class Circle(val radius: Double) : Shape {
    override fun accept(visitor: Visitor) = visitor.visit(this) // Double Dispatch
}
...
class AreaVisitor : Visitor {
    ...
    override fun visit(square: Square) { totalArea += square.side * square.side }
}
```
