...
>리모컨과 가전제품을 생각하면 쉬움. TV, 라디오마다 전용 리모컨을 따로 만드는 대신, "리모컨" 이라는 추상과 "가전제품" 이라는 구현을 다리로 연결해두면 어떤 리모컨으로도 어떤 가전제품을 제어할 수 있음 — 리모컨 종류와 가전제품 종류가 각각 따로 늘어나도 조합 폭발이 일어나지 않음.
