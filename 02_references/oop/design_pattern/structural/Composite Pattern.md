...
>택배 주문을 생각하면 쉬움. 상자 안에 또 상자가 있고, 그 안에 또 상자나 낱개 물건(망치, 폰, 충전기)이 있을 수 있음. "이 주문의 총 가격은?" 을 물을 때 상자든 낱개 물건이든 똑같이 "가격 알려줘" 라고만 물으면 됨 — 상자는 내부적으로 자기 안의 항목들에게 같은 질문을 반복해서 합산할 뿐.
