...
## 🌐 개요 (Overview)
**Zigbee**는 성숙하고 전력 소모가 적은 무선 메시 네트워킹 표준입니다. [Thread](Thread.md) 나 [Matter](Matter.md) 와 달리, 라디오부터 애플리케이션 명령까지 전체 솔루션으로 작동합니다.
...
| **Layer 3-4** (Network)   | **Zigbee NWK / APS**             | 독점 메시 라우팅 (AODV/Tree 라우팅). 보안 (AES-128) 을 포함합니다.                     |
| **Layer 2** (MAC)         | **IEEE 802.15.4 MAC**            | [Thread](Thread.md) 와 동일한 계층을 사용합니다.                                 |
| **Layer 1** (PHY)         | **IEEE 802.15.4 PHY**            | **2.4 GHz** (전 세계 공통). 간혹 Sub-GHz 대역을 사용하기도 합니다 (드묾).                |
...
"Zigbee 위에 MQTT 를 올린다"는 표현은 엄밀히 말하면 **번역 (Bridging)** 아키텍처를 의미합니다.

1. **Zigbee 구간**: 기기 ↔ 코디네이터 (언어: **ZCL**)
2. **번역기**: Zigbee2MQTT 같은 소프트웨어가 ZCL 메시지를 받아서 JSON([[MQTT]]) 으로 변환.
3. **IP 구간**: 번역기 ↔ 서버 (언어: **[[MQTT]]**, 운송: **[Wi-Fi](Wi-Fi.md)/[[Ethernet]]**)

## 🆚 기술 비교: [Zigbee](Zigbee.md) vs [Z-Wave](Z-Wave.md)

| 특징 | **Zigbee** | **Z-Wave** |
| :--- | :--- | :--- |
| **주파수** | **2.4 GHz** (Wi-Fi 와 혼잡, 전 세계 공통) | **Sub-GHz** (간섭 적음, 지역별 상이) |
| **기반 표준** | 개방형 (IEEE 802.15.4) | 독자 표준 (ITU-T G.9959) |
| **확장성** | 대규모 네트워크 (이론상 65,000+ 개) | 가정용 (232 개, 신형은 증가) |
| **속도** | 250 kbps (상대적으로 빠름) | 100 kbps (느림) |
