---
title: buffer
tags: [os, buffer, memory, data-structures]
aliases: [버퍼, Buffer, 버퍼링]
date modified: 2025-12-16 21:11:55 +09:00
...
1950~1960년대 초기 컴퓨터는 CPU와 I/O 장치의 속도 차이가 극심했다:
- **CPU**: 마이크로초(μs) 단위로 연산
- **타자기/프린터**: 문자당 수십 밀리초(ms) 소요
- **자기 테이프**: 순차 접근, 되감기에 수 초

CPU가 타자기에 직접 출력하면, 한 글자를 쓰는 동안 CPU가 수천 번의 연산을 할 수 있는 시간을 낭비했다.

**해결책**: 출력할 데이터를 메모리(버퍼)에 모아두고, CPU는 다른 작업을 계속한다. 별도의 I/O 프로세서나 DMA(Direct Memory Access)가 버퍼에서 데이터를 읽어 천천히 출력한다.
...
디스크는 블록 단위(512B, 4KB)로 읽고 쓰는 것이 효율적이다. 만약 1바이트씩 읽으면:
- 디스크 헤드 이동(seek) 비용: 수 밀리초
- 1바이트 읽기: 마이크로초

1000바이트를 읽으려면 1000번의 seek → 수 초 소요.

**버퍼 활용**: 4KB를 한 번에 읽어 버퍼에 저장. 이후 요청은 버퍼에서 반환. seek가 1회로 줄어든다.
...
**수직 동기화(VSync)**: 화면 주사율(60Hz)과 버퍼 swap을 동기화해 tearing 방지.
...
2. **TCP Transmit Buffer**: 세그먼트로 나눠 전송. ACK를 받을 때까지 재전송을 위해 보관.
...
**Flow Control**: 수신 버퍼가 가득 차면, TCP 윈도우 크기를 0으로 알려 송신측이 대기하게 한다.
...
### SurfaceFlinger의 버퍼 큐

앱이 화면을 그릴 때:
1. `dequeueBuffer()`: 빈 버퍼를 받아옴.
2. 그리기 작업 수행.
3. `queueBuffer()`: SurfaceFlinger에 제출.
4. SurfaceFlinger가 합성 후 화면 출력.
...
[[android-kernel]] - 안드로이드의 Ashmem과 DMABuf  
