...
eBPF는 **"리눅스 커널 안에서 돌아가는 자바스크립트 엔진"** 에 비유할 수 있습니다.

-   **문제점**: 예전에는 커널 기능을 추가하려면 **커널 모듈(Kernel Module)** 을 짜야 했습니다 (C언어). 하지만 모듈에 버그가 있으면 커널 전체가 멈춥니다 (Kernel Panic). 위험해서 아무나 못 했습니다.
-   **해결책 (Sandbox)**: eBPF는 **사용자가 짠 프로그램** 을 커널 내 가상머신(VM)에서 실행합니다. 단, 실행 전에 **Verifier(검증기)** 가 코드를 엄격하게 검사합니다.
    -   "무한 루프가 있는가?" (있으면 거절)
...
XDP는 eBPF 프로그램을 **네트워크 드라이버 레벨(가장 낮은 단계)** 에서 실행하는 기술입니다.
...
**XDP의 마법**:
만약 DDoS 공격 패킷이라면, 4번 `sk_buff`를 만들기 전에 **3번 단계에서 바로 버릴 수 있습니다(`XDP_DROP`).**
리눅스 커널이 무거운 네트워킹 스택을 돌릴 필요조차 없게 만드는 것입니다. 이를 통해 기존 대비 **10배 이상의 패킷 처리 성능**을 낼 수 있습니다.

예전에는 드라이버 코드를 직접 고쳐야 했지만(벤더 종속적, 위험함), 이제는 XDP를 통해 표준화된 방식으로 드라이버 단계에 개입할 수 있게 된 것입니다.
...
"서버 CPU는 왜 2개일까? 그리고 내 메모리는 왜 느릴까?"

#### 🔌 소켓(Socket)이란 무엇인가?

여기서 말하는 **소켓(Socket)** 은 네트워크 소켓(IP:Port)이 아닙니다. **CPU 칩을 꽂는 물리적인 구멍(메인보드 슬롯)** 을 말합니다.

-   **싱글 소켓(Single Socket)**: 일반적인 노트북이나 데스크탑. CPU가 1개만 꽂혀 있습니다.
-   **멀티 소켓(Multi Socket)**: 서버급 메인보드에는 CPU를 2개, 4개씩 꽂을 수 있습니다.
    -   **왜 여러 개를 꽂나?**: 하나의 CPU 칩에 코어를 무한정 늘리는 건 물리적으로 어렵습니다(발열, 크기). 그래서 CPU 칩 자체를 여러 개 꽂아 코어 수를 64개, 128개로 늘리는 방식입니다.

#### 🐢 NUMA의 문제점

멀티 소켓 서버에서는 각 CPU(소켓)마다 자기 옆에 붙어 있는 **전용 메모리(Local Memory)** 가 있습니다.

1.  **Local Access (빠름)**: CPU 1번이 자기 옆 메모리에 접근하면 아주 빠릅니다.
2.  **Remote Access (느림)**: CPU 1번이 **CPU 2번 옆에 있는 메모리**를 쓰려면, 프로세서 간 연결(QPI/UPI 등)을 건너가야 합니다. 이 과정에서 **속도가 2~3배 느려집니다.**

**커널의 역할**: 커널은 "CPU 1번에서 도는 프로세스는 무조건 CPU 1번 메모리를 쓰도록" 강제합니다. 이를 `NUMA Affinity`라고 합니다. db 서버 튜닝 시 매우 중요한 개념입니다.
