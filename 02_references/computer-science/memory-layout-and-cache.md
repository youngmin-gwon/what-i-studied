...
캐시 메모리가 효율적으로 작동하는 근간은 **"CPU가 한 번 접근한 데이터나 그 인접 데이터는 곧 다시 사용될 가능성이 높다"** 는 지역성 원리입니다.

#### 1. 시간 지역성 (Temporal Locality)
최근에 접근한 데이터에 다시 접근할 확률이 높다는 원리입니다.
- **예시**: 루프문의 카운터 변수(`i`), 자주 호출되는 재귀 함수, 누적 합을 구하는 변수.

#### 2. 공간 지역성 (Spatial Locality)
특정 데이터에 접근했을 때, 그 인접한 데이터에 접근할 확률이 높다는 원리입니다.
- **예시**: 배열 순회. CPU는 한 바이트를 읽을 때 인접한 데이터를 포함한 **Cache Line (보통 64 Bytes)** 단위로 뭉텅이로 가져옵니다.
...
OS가 제공하는 가상 메모리 기법을 통해 프로세스는 독립된 메모리 구조(Stack, Heap, Data, Text)를 가집니다. 이는 하드웨어의 물리적 배치와는 다른 논리적 추상화입니다.

> [!NOTE] 
> 상세한 영역별 구조와 OS의 관리 방식은 [가상 메모리(Virtual Memory)](../operating-systems/virtual-memory.md#가상-주소-공간-레이아웃-process-memory-layout) 문서에서 다룹니다.

---

### 🔄 데이터의 여정 (Data Journey Summary)

1. **Load**: SSD에 저장된 실행 파일이 운영체제에 의해 **RAM**으로 로드됩니다. (요구 페이징 사용)
2. **Fetch**: CPU가 데이터를 처리하기 위해 RAM의 데이터를 **캐시(L3 -> L2 -> L1)** 로 가져옵니다. 이때 지역성에 기반해 주변 데이터도 함께 가져옵니다.
3. **Execute**: 데이터가 **레지스터**에 도달하면 산술논리연산장치(ALU)가 연산을 수행합니다.
4. **Store**: 결과는 다시 역순으로 캐시를 거쳐 RAM에 반영되고, 저장이 필요하면 보조기억장치로 기록됩니다.
