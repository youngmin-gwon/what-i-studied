	...
	- Reactive Paradigm
		...
		- 목표
			- 추상화 수준을 제공하여 많은 구현 세부 사항을 처리하도록 강요하기보다는 애플리케이션에서 생성될 이벤트에 집중할 수 있도록 하는 것
			   - Instagram의 "Like" 버튼을 눌렀을 때 Like Event로 발생하는 비즈니스 로직에 집중하고 나머지 구현 세부사항은 강요하지 않음
...
- 까다로운 최적화 조건 (일부는 무슨 말인지 모르겠음)
	...
	5. enderObject는 크기를 계산하기 위해 부모가 제공한 제약 조건만 사용한다고 선언할 수 있음. 이는 제약 조건이 업데이트되는 경우를 제외하고는 이 렌더링 개체의 부모 개체를 다시 계산할 필요가 없음을 의미.