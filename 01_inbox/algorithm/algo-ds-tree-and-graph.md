...
tags: [algorithm, data-structures, tree, graph, bfs, dfs, heap]
aliases: [트리, 그래프, 이진 탐색 트리, 힙, 너비 우선 탐색, 깊이 우선 탐색]
date modified: 2025-12-18 04:30:00 +09:00
...
-   **Context**: 파일 시스템 폴더 구조, HTML DOM 트리, 회사 조직도, 그리고 **iOS의 View Hierarchy**.
-   **Recursive**: 트리는 본질적으로 재귀적입니다. "자식 뷰의 자식 뷰"를 찾는 로직은 재귀 함수 하나로 끝납니다.

### Binary Search Tree (BST)
-   **Rule**: `Left Child < Parent < Right Child`.
-   **Power**: 이 규칙 덕분에 검색 공간을 매번 절반으로 줄일 수 있습니다. (**O(log n)**). 100만 개의 데이터도 20번(`log2(1,000,000)`) 비교면 찾습니다.
...
트리보다 더 일반적인, "Node와 Edge의 집합"입니다. 순환(Cycle)이 있을 수 있습니다.
...
> [!TIP] **Context**
> "최단 경로를 찾아라" 문제가 나오면 무조건 **BFS**입니다. DFS는 해를 찾을 수는 있지만, 그게 최단이라는 보장은 없습니다.
...
### 📚 연결 문서
- [[algo-complexity-and-big-o]] - O(log n)의 위대함
- [[algo-ds-linear]] - BFS에 쓰이는 Queue 구현
- [[kernel#1. 프로세스 스케줄링]] - 힙 자료구조가 쓰이는 곳
