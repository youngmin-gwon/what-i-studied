...
aliases: [결정 트리, 선택 논리표, 알고리즘 선택 가이드]
...
#### 2. 데이터 정렬 (Sorting Strategy)

"데이터를 어떤 순서로 배치해야 후속 작업이 쉬워지는가?"

| 정렬 알고리즘 | 사용 여부 | 이유 |
|:---|:---|:---|
| **Bubble / Selection / Insertion** | ❌ **비추천** | 시간 복잡도가 $O(N^2)$ 이라 데이터가 조금만 많아져도(N>5,000) 시간 초과가 발생합니다. |
| **Quick / Merge / Heap Sort** | ⚙️ **개념적 중요** | 현대 언어의 내장 함수(`sort()`)가 이들을 기반으로 구현되어 있습니다. 직접 구현할 일은 거의 없습니다. |
| **내장 sort() 함수 (Timsort)** | ✅ **적극 권장** | 가장 빠르고 안정적($O(N \log N)$)입니다. **정렬이 필요하면 무조건 이걸 쓰세요.** |

##### 💡 정렬이 "먼저" 필요한 상황 (Pre-processing)

- **이진 탐색**을 하기 직전
- **그리디(Greedy)** 알고리즘으로 접근할 때 (가장 큰/작은 것부터 처리)
- **투 포인터**나 **슬라이딩 윈도우**를 적용할 때
- 중복된 데이터를 제거하거나 그룹화할 때

---

#### 3. 그래프와 네트워크 (Graph & Connectivity)
...
**정본 참고**: 입력 크기별 허용 복잡도와 구체적인 근거는 [복잡도 분석](complexity-and-big-o.md)의 "입력 크기($N$)에 따른 알고리즘 선택 가이드" 섹션을 참조하세요.
...
- [문제 해결 프로세스](problem-solving-process.md) - 설계 단계의 4 단계 프레임워크
