...
대부분의 온라인 저지 환경에서 **1초는 약 1억 번의 연산**을 의미합니다.
- 내 풀이가 $N=10^5$ 일 때 $O(N^2)$ 이라면? $10^{10}$ 번 연산 $\rightarrow$ 약 100초 소요 $\rightarrow$ **TLE (시간 초과) 확정**.
- 이럴 때는 $O(N \log N)$ 이하의 알고리즘을 찾아야 합니다.

### 2. 메모리 제한과 공간 계산

공간 복잡도는 보통 여유롭지만, 대형 배열을 선언할 때 주의해야 합니다.
- **int 자료형 기준**: 1MB $\approx$ 약 25만 개의 정수 저장 가능.
- **안전선**: 보통 128MB~512MB 제한 환경에서 **5,000만 개 이하**의 공간을 할당한다면 OOM(메모리 초과) 위험에서 안전합니다.
...
- **공간 최적화 (Generator)**: 1억 개의 숫자를 리스트로 만들면 메모리가 부족해지지만, `yield`를 사용하는 **제너레이터**는 값을 필요할 때만 생성하여 공간 복잡도를 $O(1)$로 유지할 수 있습니다.

> [!IMPORTANT] **한 줄 정리**
> - **연산 횟수 1억 번 이하** $\rightarrow$ 시간 초과 가능성 매우 낮음.
> - **공간 할당 5,000만 개 이하** $\rightarrow$ 메모리 초과 가능성 매우 낮음.
