...
많은 사람이 Big-O를 '최악의 경우'와 혼용하지만, 둘은 수리적 개념과 시나리오적 개념으로 구분됩니다.
...
> "Quick Sort의 시간 복잡도는 얼마인가요?"라는 질문을 받는다면 다음과 같이 답변하는 것이 가장 완벽합니다.
> 
...
2. **상수항의 이점**: Quick Sort는 $O(N \log N)$ 알고리즘 중에서도 상수항이 매우 작아 실질적으로 Merge Sort보다 빠릅니다.
3. **실무적 해결**: 최신 표준 라이브러리의 `sort()` 함수들은 Quick Sort로 시작하되, 재귀 깊이가 너무 깊어지면 Heap Sort로 전환하는 **Introsort** 등을 사용하여 최악의 경우에도 $O(N \log N)$을 보장하도록 개량되어 있습니다.
