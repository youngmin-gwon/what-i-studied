...
#### 1. 약수 집합 활용 (Naive)
두 수의 모든 약수를 구한 뒤, 공통된 약수 중 최대값을 찾습니다. 직관적이지만 $O(\sqrt{a} + \sqrt{b})$ 의 비용이 발생합니다.
```python
def get_gcd_naive(a, b):
    # get_divisors 함수는 math-prime-number 문서 참고
    set_a = set(get_divisors(a))
    set_b = set(get_divisors(b))
    return max(set_a & set_b) # 교집합(공약수) 중 최대값
```
...
- [복잡도 분석](../00_fundamentals/complexity-and-big-o.md) - 유클리드 호제법의 로그 시간($O(\log N)$) 효율성 이해
- [검색과 정렬](../02_algorithms/search-and-sort.md) - 정렬된 데이터를 다루는 배열 회전(Array Rotation) 응용
- [배열과 리스트](../01_data-structures/linear.md) - 배열의 메모리 레이아웃과 포인터 연산의 기초
- [소수와 수론](math-prime-number.md) - 최대공약수가 1인 서로소(Coprime) 관계의 중요성
- [나머지 연산](math-modular-and-exponentiation.md) - 거대 숫자의 연산과 모듈러 역원 계산의 토대
