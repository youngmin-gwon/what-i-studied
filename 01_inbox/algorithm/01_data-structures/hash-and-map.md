...
### 1. 딕셔너리 (Dictionary)
Python의 `dict`는 매우 효율적인 해시 테이블로 구현되어 있습니다. (Python 3.7+부터는 삽입 순서가 보장됩니다.)

#### 💻 주요 연산 및 복잡도
| 연산 | 코드 | 시간 복잡도 | 비고 |
| :--- | :--- | :--- | :--- |
| **조회/삽입/삭제** | `dt[key]`, `del dt[k]` | $O(1)$ | 평균 케이스 |
| **키 존재 확인** | `key in dt` | $O(1)$ | 해시를 통한 즉시 확인 |
| **모든 키 리스트** | `dt.keys()` | $O(1)$ | 뷰(View) 객체 반환 |

### 2. 세트 (Set)
`set`은 사실상 Value가 없는 딕셔너리로, 중복을 허용하지 않는 집합 연산에 최적화되어 있습니다.

#### ⏱️ 집합 연산 복잡도
- **합집합 (`s1 | s2`)**: $O(N+M)$
- **교집합 (`s1 & s2`)**: $O(min(N, M))$
- **차집합 (`s1 - s2`)**: $O(N)$ (s1의 크기에 비례)
