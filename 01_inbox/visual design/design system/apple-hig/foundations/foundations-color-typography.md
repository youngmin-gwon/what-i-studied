...
## 🌈 Color & Material System (색상과 머티리얼)

Apple 의 컬러는 단순히 색을 고르는 것이 아니라, **의미(Semantics)** 와 **물성(Material)** 을 부여하는 과정입니다.

### 1. Liquid Glass (리퀴드 글래스): 최신 디자인의 정수

- **개념**: iOS 18 과 visionOS 에서 더욱 강조된 스타일로, 단순한 블러를 넘어 빛의 굴절, 표면의 광택(Specular), 그리고 깊이감을 실시간으로 계산하는 동적 머티리얼입니다.
- **역할**: 배경 콘텐츠와의 조화를 유지하면서도, 제어 요소(Tab Bar, Sidebar)가 콘텐츠 레이어 위에 떠 있는 **기능적 층위**임을 명확히 합니다.

### 2. Materials (머티리얼 계층)

Apple 은 주변 환경에 따라 투명도와 블러 두께를 조절하는 4 가지 표준 머티리얼을 제공합니다.
- **Ultra-thin**: 배경이 가장 많이 비침. 보조적인 레이어나 가벼운 UI 에 사용.
- **Thin**: 시스템 기본 스타일. 적절한 가독성과 배경 투영의 균형.
- **Regular**: 알림(Alerts)이나 다이얼로그처럼 정보 전달이 최우선일 때 사용.
- **Thick**: 배경을 거의 차단하여 가장 높은 대비와 가독성을 보장.

### 3. Vibrancy (바이브런시)

- **효과**: 배경의 색상을 전경으로 끌어올려(Pulling color), 어떤 배경 위에서도 텍스트와 아이콘이 묻히지 않고 판독성을 유지하게 합니다.
- **Layering**: Vibrancy 는 반드시 Material 위에서 작동해야 하며, 텍스트 가독성을 위한 WCAG 4.5:1 대비를 동적으로 보장합니다.
