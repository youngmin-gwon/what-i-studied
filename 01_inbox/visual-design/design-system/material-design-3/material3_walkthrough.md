...
**Material Design 3 (M3)** "Material You"라는 개인화 철학을 넘어, **"Expressive(표현형)"** 업데이트를 통해 인간의 감성과 기기의 반응성 사이의 간극을 좁혔습니다. 단순히 '예쁜' 디자인을 넘어, 디자인이 사용자에게 어떻게 **말을 거는지**, 그리고 어떻게 **정서적 가치** 를 제공하는지에 집중합니다.
...
Material You 의 핵심은 사용자가 디자인의 공동 제작자가 된다는 것입니다.

### Dynamic Color & Tonal Palettes

단순히 배경색이 바뀌는 것이 아닙니다. M3 알고리즘은 사용자의 배경화면에서 핵심 색상을 추출하고, 이를 5 개의 톤 범위(Tonal Palettes)로 나눕니다.

![dynamic_color_gen](../../../../_assets/material_3/dynamic_color_gen.png)

*<사용자 배경에서 추출된 Source Color 가 시스템 전체의 Tonal Palette 로 확장되는 과정>*
...
![expressive_motion](../../../../_assets/material_3/expressive_motion.png)
...
- **Corner Radii Tokens**: 10 단계 이상의 세분화된 곡률 토큰을 통해 브랜드 고유의 '리듬'을 형성합니다.
...
- **M3 Actions: 표현력이 풍부한 버튼과 위계**
- **M3 Containment: 몰입과 적응형 레이아웃**
- **M3 Selection: 직관적인 입력과 동적 피드백**
- **Selection & Inputs**: 슬라이더(Sliders)와 입력 도구.
- **Feedback**: 로딩(Loading)과 진행률(Progress) 인디케이터.
- **Search**: 검색바(Search Bar)와 검색 뷰(Search View).
- **Communication**: 다이얼로그, 시트, 툴팁 및 알림.
- **M3 Typography: 의미론적 가독성과 폰트 역할**
- **Color & Theme**: HCT 컬러 시스템과 다이나믹 컬러.

---

## 🏗️ 적응형 및 몰입형 레이아웃 (Adaptive & Immersive)

M3 Expressive 는 모바일을 넘어 **XR(AR/VR)** 및 대화면 기기로 확장됩니다.
