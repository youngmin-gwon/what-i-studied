...
aliases: []
date modified: 2025-12-11 13:59:39 +09:00
...
**제 4 장: Dynamic Routing Protocols (동적 라우팅 프로토콜)**
...
- **부하 분산 (Load Balancing):** 동일한 목적지로 가는 여러 경로에 트래픽을 분산시키는 것입니다. 대부분의 프로토콜은 동일 비용 (Equal-cost) 부하 분산을 지원하며, EIGRP 와 같은 일부 프로토콜은 불균등 비용 (Unequal-cost) 부하 분산도 지원합니다.
...
- **루프 방지 및 안정화 메커니즘:**
    ...
    - **Counting to Infinity:** 루프 발생 시 홉 카운트가 무한히 증가하는 것을 막기 위해, 최대 홉 (예: RIP 의 경우 16) 을 정의하여 이를 초과하면 도달 불가능으로 간주합니다.
...
**Dijkstra 의 SPF(Shortest Path First) 알고리즘**을 사용합니다. 각 라우터는 네트워크 전체의 연결 상태 정보 (Link-State) 를 수집하여 **위상 데이터베이스 (Topological Database)**를 구축합니다. 이는 전체 지도를 가지고 직접 경로를 계산하는 것과 같아 **"도로 지도 (Road map)"**에 비유됩니다.
...
- **플러딩 제어:** LSA 의 중복 처리를 막고 최신 정보를 유지하기 위해 **Sequence Numbers(순서 번호)**와 **Aging(에이징)**을 사용합니다. 순서 번호 공간으로는 Linear, Circular, Lollipop 방식 등이 있습니다,.
- **영역 (Areas):** 대규모 네트워크에서 데이터베이스 크기와 SPF 계산 부하를 줄이기 위해 네트워크를 계층적 영역으로 나눕니다. 백본 영역 (Area 0) 이 다른 영역들을 연결합니다,.

### 4. IGP 와 EGP (Interior and Exterior Gateway Protocols)

라우팅 프로토콜은 자율 시스템 (Autonomous System, AS) 을 기준으로 분류됩니다. AS 는 공통된 관리 하에 있는 네트워크 집합입니다.

- **IGP (Interior Gateway Protocol):** AS 내부에서 사용됩니다 (예: RIP, EIGRP, OSPF, IS-IS).
- **EGP (Exterior Gateway Protocol):** AS 간의 라우팅에 사용됩니다 (예: BGP).
- 참고: IGRP/EIGRP 에서는 AS 번호를 프로세스 도메인을 식별하는 용도로 사용하기도 합니다,.
