---
title: android-keystore-and-biometrics
tags: [android, security, keystore, biometric]
aliases: [Android 보안 저장소, 생체 인증]
date created: 2026-04-05 12:40:00 +09:00
---

## Android Keystore & Biometrics (Production Guide)

본 문서는 상용 서비스에서 민감 정보(사용자 토큰, 서명 키 등)를 안전하게 보호하기 위한 **Android Keystore** 하드웨어 바인딩 전략과 **BiometricPrompt** 연동 실무를 다룹니다.
...
안드로이드 10+ 부터는 **BiometricPrompt**를 통해 일관된 UI와 보안 등급별 제어를 수행합니다.
...
1. **중요 데이터 저장 금지**: Keystore 에 키를 저장하되, **서버 URL이나 비즈니스 로직에 기반한 고정된 값**은 소스 코드에 하드코딩하지 말고 환경변수나 TEE 내부에서 파생된 키를 사용해야 합니다.
