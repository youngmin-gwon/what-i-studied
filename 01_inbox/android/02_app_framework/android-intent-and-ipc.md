...
tags: [android, android/intent, android/ipc, android/pending-intent, android/deep-link]
aliases: [Android Intent, PendingIntent, Intent Filter, Activity Result API]
date modified: 2026-04-04 00:11:00 +09:00
date created: 2026-04-04 00:11:00 +09:00
---

## Android Intent & 앱 간 통신 Deep Dive

Intent 는 안드로이드의 **메시징 프레임워크**다. 컴포넌트(Activity, Service, BroadcastReceiver)를 시작하고, 앱 간/앱 내 데이터를 전달하는 유일한 표준 메커니즘이다.

> [!NOTE] **iOS 비교: Intent vs URL Scheme**
> Android 의 Intent 시스템은 iOS 에서 직접적으로 대응하는 단일 개념이 없다. iOS 에서는 **URL Scheme** (앱 간 통신), **Universal Links** (웹→앱), **XPC** (프로세스간 통신), **App Intents** (시스템 통합)이 각각 분리되어 있지만, Android 는 이 모든 역할을 **Intent 하나로 통합**한다.
> 자세한 내용은 [apple-interprocess-and-xpc](../../apple/04_system_services/apple-interprocess-and-xpc.md), [apple-app-intents](../../apple/04_system_services/apple-app-intents.md) 참고.
