...
aliases: [Accessibility, Compose Accessibility, Semantics, TalkBack]
date modified: 2026-04-05 12:25:22 +09:00
date created: 2026-04-04 00:28:00 +09:00
---

## Android Accessibility with Compose

안드로이드 접근성(Accessibility)은 장애가 있는 사용자를 포함한 모든 사용자가 앱을 동등하게 사용할 수 있도록 보장하는 기술이다. Jetpack Compose 는 **Semantics Tree**라는 선언형 구조를 통해 접근성 정보를 관리한다.

>[!NOTE] **iOS 비교: Accessibility Tree vs Semantics Tree**
> - **iOS**: SwiftUI 는 코드를 통해 레이아웃을 정의하면 시스템이 자동으로 **Accessibility Tree**를 생성하며, 프레임워크 수준에서 묵시적(Implicit)으로 처리되는 부분이 많다.
> - **Android**: Compose 는 시각적 트리와 별개로 명시적인 **Semantics Tree**를 구축한다. 개발자가 `Modifier.semantics` 를 통해 정보를 직접 제어하며, 테스트 도구와 접근성 서비스(TalkBack)가 이 트리를 활용한다.
