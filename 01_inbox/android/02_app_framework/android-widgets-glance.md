...
tags: [android, android/widgets, android/glance, android/remoteviews]
aliases: [App Widgets, Jetpack Glance, RemoteViews, Home Screen Widgets]
...
안드로이드의 홈 화면 위젯은 단순한 정보를 한눈에 제공하는 도구이다. **Jetpack Glance**는 Jetpack Compose의 선언형 모델을 위젯 개발에 도입하여 `RemoteViews` 기반 개발의 복잡성을 획기적으로 줄였다.

> [!NOTE] **iOS 비교: WidgetKit vs Jetpack Glance**
> - **iOS**: `WidgetKit` (SwiftUI 기반)을 통해 홈 화면과 잠금 화면에 동적인 위젯을 배치한다. (iOS 14+)
> - **Android**: `Jetpack Glance` (Compose 기반)를 통해 홈 화면 위젯을 빌드한다. 두 플랫폼 모두 선언형 UI를 통해 데이터를 효율적으로 표시하며, 시스템 리소스 사용을 최소화하도록 설계되었다.
> 자세한 내용은 [apple-widgets-live-activities](../../apple/02_ui_frameworks/apple-widgets-live-activities.md)를 참고하세요.
...
- [android-background-processing](android-background-processing.md)
- [android-persistence-room-datastore](android-persistence-room-datastore.md)
