...
## 소스 준비
- repo init/sync로 manifest 기반 fetch. shallow clone 시 특정 브랜치 depth 관리. local manifest로 사내 미러/프로프라이어터리 blob 포함.
...
- Make에서 Soong으로 전환 진행; Blueprint/soong_config_variables로 제품별 설정. APEX/APK/CC/Java 모듈 타입.
...
- apex payload는 dm-verity hash tree 포함. `apexd`가 install/activate 관리, rollback data 유지.
