...
* **네비게이션 GPS 길 찾기**:
  - 목적지 주소(IP)는 알지만, 어떤 길로 가야 빠를지 모른다.
  - GPS(라우터)가 현재 도로 상황을 보고 "강남대로 → 영동대로 → 신월로"라고 경로를 제시한다. → **라우팅 테이블**
  - 도중에 도로 공사가 있으면 자동으로 "우회 경로로 가세요"라고 안내한다. → **동적 라우팅 (경로 학습)**
  - 매번 목적지에 도착할 때마다 경로를 다시 계산한다. → **각 라우터에서의 홉 단위 결정**
