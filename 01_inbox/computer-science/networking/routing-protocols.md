...
라우팅 프로토콜은 라우터 간 경로 정보를 교환하여 **최적 경로를 자동으로 결정**하는 프로토콜입니다. AS(Autonomous System: 자율 시스템) 내부냐 외부냐에 따라 **IGP**와 **EGP**로 분류됩니다.
...
* **신문 배달원들의 정보 공유 시스템**:
  - **RIP (거리 벡터)**: 배달원 A가 B에게 "나는 A지역 배달을 하고 5번의 골목을 지나야 B지역에 도착해"라고 말한다. 하지만 전체 도시 지도는 모른다.
  - **OSPF (링크 상태)**: 배달원들이 정기적으로 모여 전체 도시 지도를 공유하고, 각자 최적 경로를 계산한다.
  - **BGP (경로 벡터)**: 서로 다른 신문사들이 만났을 때 "우리는 이 경로를 거쳐서 배달하니까 너희도 이용해도 괜찮다"고 정책에 따라 선택적으로 정보를 공유한다.
