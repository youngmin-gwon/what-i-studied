...
* **비순수 함수 (오늘의 운세 자판기 / 주사위 던지기)**:
  - 동전을 넣어도 시각, 현재 날씨, 전역 상태 변수 등에 따라 매번 결과가 달라진다. 또한 버튼을 누를 때마다 사장님 핸드폰으로 문자 메시지가 발송([Side Effect](side-effect.md))된다. (결과 불확실, 외부 상태 변이)
...
| **부작용 ([Side Effect](side-effect.md))** | **❌ 전혀 없음** | **⭕ 존재 함** (전역 변수 수정, DB 변경, I/O 수행) |
...
현대 소프트웨어 아키텍처(Clean Architecture, Redux, Jetpack Compose 등)에서는 **"비즈니스 로직과 UI 렌더링은 순수 함수로 작성하고, [Side Effect](side-effect.md)는 격리된 영역(Effect Handling Layer)에서 제어"**하는 패턴을 채택한다.
...
- [Side Effect](side-effect.md) - 부작용의 정의 및 소프트웨어 영향
