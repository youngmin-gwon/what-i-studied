...
tags: ["jvm", "classloader", "loading", "linking", "initialization", "symbolic-reference", "direct-reference", "delegation-model", "computer-science"]
aliases: ["클래스로더", "ClassLoader", "JVM ClassLoader", "클래스 로딩 메커니즘", "위임 모델", "심볼릭 참조", "직접 참조"]
date created: 2026-08-19 15:05:00 +09:00
date modified: 2026-08-19 16:20:00 +09:00
---
...
flowchart TD
    subgraph "ClassLoader 3단계 생명주기"
        L1["1. Loading (로딩)<br/>.class 바이너리를 읽어 Metaspace 에 메타데이터 생성"] --> L2["2. Linking (링킹)<br/>Verification ➔ Preparation ➔ Resolution"]
        L2 --> L3["3. Initialization (초기화)<br/>정적 변수 실제 값 할당 및 static {} 블록 실행"]
    end
```
...
- OS가 `java` 명령을 실행하면 C/C++로 구현된 JVM 네이티브 엔진이 초기화된다.
...
- **주요 검증 항목**:
  1. 파일의 첫 4바이트가 Java 바이트코드 매직 넘버인 `0xCAFEBABE`로 시작하는가?
...
- **기본 초깃값(Default Zero Value) 할당**:
  - 코드에 `public static int timeout = 5000;`이라고 작성되어 있더라도, 이 단계에서는 `5000`을 넣지 않고 **`0` (참조형은 `null`, boolean은 `false`)**을 기록한다.
...
- **격리 메커니즘**:
  - 플러그인 A와 플러그인 B가 서로 다른 버전의 라이브러리(`Guava 28.0` vs `Guava 32.0`)를 사용하더라도, 각 플러그인을 서로 다른 `Custom ClassLoader` 인스턴스에 로드한다.
  - 가시성 원칙에 의해 로더 A와 로더 B는 서로의 클래스를 볼 수 없으므로, 하나의 JVM 프로세스(Gradle 데몬) 안에서 버전 충돌 없이 안전하게 공존한다.
