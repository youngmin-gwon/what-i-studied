...
<details>
<summary>조직 내부의 모든 사용자와 디바이스를 기본적으로 신뢰하지 않고(Never Trust), 네트워크 내·외부에 관계없이 리소스에 접근할 때마다 철저한 신원 확인과 검증(Always Verify)을 요구하는 최신 보안 패러다임의 명칭을 쓰시오.</summary>
<blockquote>
제로 트러스트 (Zero Trust)
</blockquote>
</details>
...
<details>
<summary>디지털 포렌식 시 증거 수집 과정에서 지켜야 할 '원본 유지의 원칙'의 개념을 설명하고, 사건사고 조사 시 원본 하드디스크를 복사하여 분석할 때 원본과의 동일성을 입증하기 위해 필수적으로 문서화해야 하는 암호학적 기법 한 가지를 쓰시오.</summary>
<blockquote>
