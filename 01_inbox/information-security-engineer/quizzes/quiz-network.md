...
- <strong>해설</strong>:
  - **B 클래스**: 전체 32비트 중 앞의 16비트가 네트워크 영역(10 + 14bit)이며, 뒤의 **16비트가 호스트 영역**입니다. 따라서 총 IP 개수는 $2^{16}(65,536)$개이며, 여기서 **네트워크 주소**와 **브로드캐스트 주소** 2개를 제외한 65,534개가 실제 할당 가능한 IP 수가 됩니다.
  - **C 클래스**: 전제 32비트 중 앞의 24비트가 네트워크 영역(110 + 21bit)입니다. 서브넷 마스크는 네트워크 비트 부분을 모두 '1'로 채운 값이므로, 11111111(255)이 세 번 반복되는 **255.255.255.0**(/24) 형식을 갖습니다.
</blockquote>
</details>
...
- <strong>해설</strong>: <code>133.253.27.45</code>의 첫 번째 옥텟인 **133**은 B 클래스 범위(128~191)에 속하므로, 기본 서브넷 마스크는 <code>255.255.0.0</code>(/16)이 된다. 따라서 앞의 2개 옥텟이 네트워크 주소가 되며, 나머지를 0 또는 255로 채우면 각각 네트워크 주소와 브로드캐스트 주소가 도출된다.
</blockquote>
</details>
...
2. <strong>Linux / Unix / macOS</strong> 계열: [ ( B ) ]</summary>
<blockquote>
(A) <strong>128</strong><br>
...
- <strong>헤더 단순화</strong>: 검사합(Checksum) 필드를 삭제하여 라우팅 지연을 줄임 (Checksum 처리는 L2와 L4에 위임함)<br>
