...
4. **파티션 이미지 생성 (Image Packing & AVB Signing)**:
   ...
   - Dynamic Partition 환경에서는 `lpmake` 도구가 `system`, `vendor`, `product` 등을 통합한 `super.img`를 생성하고, **AVB**(Android Verified Boot — 부트로더가 각 파티션 이미지의 서명과 해시를 검증해 변조된 이미지로는 부팅하지 못하게 막는 체계. 정식 정의는 [AVB는 부팅 이미지의 신뢰와 rollback 방지를 검증한다](../boot-and-runtime/boot-flow/android-verified-boot.md) 참고) 툴인 `avbtool`로 해시 풋터를 추가한다.
...
관련 노트: [Device bring-up은 board, kernel, HAL, VINTF, sepolicy 통합이다](device-bringup-integration.md), [product configuration은 package, property, permission, overlay를 선택한다](product-makefiles-configuration.md).
