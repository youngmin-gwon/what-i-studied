...
1. **[Binder 커널 드라이버](ipc-and-process/ipc-process/binder-kernel-driver.md)**: 커널 드라이버 및 mmap 1 회 복사 메모리 메커니즘
2. **[Binder 프레임워크](ipc-and-process/ipc-process/binder-framework.md)**: libbinder, Proxy/Stub, Parcel, ServiceManager
3. **[Binder 트랜잭션 수명과 1MB 버퍼 제한](ipc-and-process/ipc-process/binder-transaction-lifetime-is-call-copy-dispatch-and-reply.md)**: Call ➔ Copy ➔ Dispatch ➔ Reply 4 단계 및 `TransactionTooLargeException`
4. **[Binder 스레드 풀과 서비스 동시성 경계](ipc-and-process/ipc-process/binder-thread-pool-is-service-concurrency-and-deadlock-boundary.md)**: 수신 스레드 풀(최대 16 개) 및 중첩 동기 호출 시 Deadlock 예방
5. **[Oneway 비동기 바인더 통신](ipc-and-process/ipc-process/oneway-binder-removes-caller-waiting-not-server-backpressure.md)**: `oneway` 키워드 비동기 통신과 서버 백프레셔(Backpressure) 처리
...
- [Binder 커널 드라이버](ipc-and-process/ipc-process/binder-kernel-driver.md)
- [Binder 유저스페이스 프레임워크](ipc-and-process/ipc-process/binder-framework.md)
