...
**Binder IPC**는 Linux 커널 드라이버(`/dev/binder`)와 Android 유저스페이스 프레임워크(`libbinder`)를 기반으로, 서로 다른 프로세스 간(예: 앱 프로세스 ↔ [system_server](../boot-and-runtime/system-server/system-server.md)) 메모리를 완벽히 격리한 채 고속으로 데이터와 원격 명령을 주고받는 **Android OS 핵심 IPC 아키텍처**이다.
...
| **중앙 서비스 등록소** | DNS, 포트, 파일 경로 | **[ServiceManager](../../04_system_services/service-lookup/service-manager.md) (Handle 0)** |
...
- [system_server](../boot-and-runtime/system-server/system-server.md)
