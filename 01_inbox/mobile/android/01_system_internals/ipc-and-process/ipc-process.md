...
- [Binder transaction lifetime은 call, copy, dispatch, reply로 나뉜다](binder-transaction-lifetime.md)
- [AIDL은 process boundary 계약이지 비즈니스 프로토콜이 아니다](aidl-process-boundaries.md)
- [oneway Binder는 caller 대기를 없애지만 server backpressure를 없애지 않는다](oneway-binder-transactions.md)
- [Binder thread pool은 service concurrency와 deadlock 경계다](binder-thread-pool.md)
- [IPC 디버깅은 service 등록, call path, thread state에서 시작한다](ipc-service-debugging.md)
...
- [Zygote fork의 메모리 이점은 copy-on-write가 유지될 때 생긴다](../boot-and-runtime/zygote-runtime/zygote-copy-on-write.md)
- [system_server는 framework service를 한 process 안에서 시작한다](../boot-and-runtime/system-server/system-server-startup.md)
- [system service는 Binder endpoint이자 platform policy enforcer다](../boot-and-runtime/system-server/system-service-binder-endpoint.md)
- [프로세스 우선순위는 메모리 회수 정책 입력이지 앱 상태의 진실이 아니다](../boot-and-runtime/system-server/process-priority-oom-score.md)
- [LMKD는 free memory가 아니라 memory pressure와 process importance로 종료를 결정한다](../kernel-and-hal/kernel/lmkd-memory-pressure.md)
...
- SurfaceFlinger, camera service, mediaserver 같은 native graphics/media service 의 buffer/session 계약은 [Graphics and media contracts](../graphics-and-media/graphics-media.md) 가 정본이다. BufferQueue 는 버퍼 핸들을 프로세스 간에 넘기는 자료구조이고, 그 등록/제어 채널은 Binder 를 쓴다.
- ConnectivityService, netd 같은 network system service 의 정책 계약은 [연결성 계약](../connectivity/connectivity.md) 이 정본이다. 앱이 보는 `ConnectivityManager` 호출도 결국 Binder 를 통해 system_server 의 ConnectivityService 로 전달된다.
- system_server 가 여러 framework service 를 한 프로세스에서 어떻게 조율하는지는 [system_server와 ActivityManager 계약](../../04_system_services/system-server.md) 이 정본이다.
