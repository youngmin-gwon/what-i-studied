...
aliases: []
date modified: 2026-08-03 17:24:32 +09:00
...
Default network 는 시스템이 앱 트래픽에 기본 적용하는 네트워크다. `requestNetwork()` 로 요청한 네트워크는 앱이 특정 capability 를 요구해 시스템에 연결 유지를 요청하는 별도 수명 계약이며, 권한과 배터리 비용을 동반할 수 있다.

### 실무 규칙

- 일반 HTTP 호출은 앱 default network 를 사용한다.
...
- 요청이 끝나면 반드시 callback 을 unregister 해 네트워크 유지와 callback 제한을 해제한다.
- mutable capability 인 `VALIDATED`, `CAPTIVE_PORTAL` 은 request 조건으로 부적절하다.
