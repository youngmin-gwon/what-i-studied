...
Private DNS는 시스템 DNS resolver가 DNS-over-TLS로 질의를 암호화하도록 하는 플랫폼 기능이다. 이는 DNS 감청과 변조 위험을 줄이지만, 앱의 HTTPS/TLS 검증, certificate pinning, backend authentication을 대체하지 않는다.

## 판단 기준

- 사용자가 설정한 Private DNS provider는 앱이 임의로 통제하지 않는다.
- 앱은 `LinkProperties.isPrivateDnsActive()`와 `getPrivateDnsServerName()`으로 현재 네트워크의 Private DNS 상태를 관찰할 수 있다.
- DNS-over-TLS가 켜져도 SNI, IP, traffic pattern 등 다른 metadata가 모두 사라지는 것은 아니다.
- 앱의 보안 연결 정책은 Network Security Config와 TLS library 설정에서 다룬다.
