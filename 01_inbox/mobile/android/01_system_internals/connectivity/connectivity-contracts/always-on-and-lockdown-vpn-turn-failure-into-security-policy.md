...
Always-on VPN은 부팅 후 VPN service를 계속 유지하려는 시스템 정책이고, lockdown VPN은 VPN을 우회하는 트래픽을 차단하는 더 강한 정책이다. 이 모드에서는 VPN 연결 실패가 단순 네트워크 오류가 아니라 전체 앱 연결 차단으로 이어질 수 있다.

## 실무 규칙

- always-on을 지원하지 못하는 VPN 앱은 manifest metadata로 명시적으로 opt-out한다.
- lockdown 모드에서는 allowlist/disallowlist와 enterprise policy가 사용자 네트워크 경험을 크게 바꾼다.
- VPN service는 앱 업데이트, reboot, network handover 이후 재연결을 견뎌야 한다.
