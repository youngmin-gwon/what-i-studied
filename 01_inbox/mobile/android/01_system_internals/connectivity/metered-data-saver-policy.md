---
title: metered-data-saver-policy
...
2. **Data Saver Status (`RESTRICT_BACKGROUND`)**:
   ...
   - 앱이 백그라운드(`PROCESS_STATE_BACKGROUND`)로 전환되는 즉시, `netd`는 **eBPF/iptables**(커널 안에서 UID별로 패킷을 필터링하는 Linux 방화벽 메커니즘 — 자세한 정의는 [netd 문서](netd-daemon.md) 참고) `bw_penalty_box` 룰에 해당 앱의 UID를 추가하여 소켓 연결 수신/발신 시 `EPERM` 또는 `EACCES` 에러를 반환한다.
...
- [셀룰러 연결성은 요금제와 시스템 정책에 의해 통제된다](cellular-connectivity-policy.md)
