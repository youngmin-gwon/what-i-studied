...
관련 노트: {link(contracts_hub / "audiofocus-is-shared-output-policy-not-playback-permission.md", "AudioFocus는 재생 권한이 아니라 공유 출력 정책이다")}, {link(contracts_hub / "drm-protected-media-needs-secure-codec-and-output-path.md", "DRM 보호 미디어는 secure codec과 보호된 출력 경로를 요구할 수 있다")}
