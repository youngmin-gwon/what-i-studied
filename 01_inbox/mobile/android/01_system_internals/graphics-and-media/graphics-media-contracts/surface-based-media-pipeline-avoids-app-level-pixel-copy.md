...
관련 노트: {link(contracts_hub / "bufferqueue-separates-producer-and-consumer-with-buffer-ownership.md", "BufferQueue는 producer와 consumer를 버퍼 소유권으로 분리한다")}, {link(contracts_hub / "drm-protected-media-needs-secure-codec-and-output-path.md", "DRM 보호 미디어는 secure codec과 보호된 출력 경로를 요구할 수 있다")}
