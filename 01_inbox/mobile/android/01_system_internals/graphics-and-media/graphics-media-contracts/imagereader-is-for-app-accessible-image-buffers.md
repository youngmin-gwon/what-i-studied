...
ImageReader의 `maxImages`는 동시에 앱이 잡고 있을 수 있는 이미지 수의 상한이다. 이 상한을 넘기면 새 이미지를 얻지 못하거나 producer가 stall/drop 상태가 될 수 있다.
...
관련 노트: {link(contracts_hub / "camera-output-surfaces-define-preview-analysis-and-recording-pipelines.md", "카메라 출력 Surface는 프리뷰, 분석, 녹화 파이프라인을 정의한다")}
