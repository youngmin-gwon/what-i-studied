...
관련 노트: {link(contracts_hub / "surface-based-media-pipeline-avoids-app-level-pixel-copy.md", "Surface 기반 미디어 파이프라인은 앱 수준 픽셀 복사를 줄인다")}, {link(contracts_hub / "mediacodec-bytebuffer-mode-makes-the-app-own-sample-flow.md", "MediaCodec ByteBuffer 모드는 앱이 sample 흐름을 소유한다")}
