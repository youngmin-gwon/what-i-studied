...
**Generic Kernel Image**(GKI — Google 이 배포하는 단일 공용 코어 커널 바이너리를 모든 벤더가 공유하고, 칩셋별 드라이버는 별도 모듈로 분리하는 정책. 배경은 아래 관련 문서의 GKI 노트 참고) 환경에서 범용 코어 커널(`boot.img`)은 SoC 칩셋 특화 드라이버나 스토리지/디스플레이 컨트롤러 드라이버를 커널 바이너리에 직접 내장하지 않는다.

대신 부팅 초기 필수 드라이버 모듈(`.ko`)은 **vendor_boot 파티션**(벤더가 제공하는, 부팅 초기에만 필요한 드라이버·fstab 등을 담은 전용 파티션 — GKI 도입으로 `boot.img` 에서 벤더 종속 요소를 분리하며 생겼다)의 **First-stage RAMDisk**(커널이 맨 처음 마운트하는 임시 root 파일시스템 이미지. 실제 `system`/`vendor` 파티션들을 마운트하기 전까지 필요한 최소 바이너리만 담는다)에 탑재되며, 부팅 1단계 프로세스인 **First-stage init**(root 파일시스템이 아직 완전히 마운트되지 않은 시점에 실행되는 init 의 첫 단계 — 두 단계로 나뉘는 이유는 [First-stage init builds minimal filesystem](../../boot-and-runtime/init-service/first-stage-init-builds-minimal-filesystem-for-second-stage.md) 참고)이 **modules.load**(로드할 모듈 목록을 순서대로 나열한 파일) 및 **modules.dep**(모듈 간 의존성 순서를 정의한 파일) 종속성 순서에 따라 커널 동적 모듈 로드(`modprobe` / `init_module` — `.ko` 파일을 런타임에 커널로 적재하는 도구/syscall)를 수행한다.
...
- 커널 모듈 작성 시 `MODULE_SOFTDEP("post: msm_drm")`(커널 모듈 간 로드 순서 의존성을 선언하는 매크로) 선언을 활용하여 모듈 간 의존성 관계를 타이트하게 정의해야 init 이 정방향으로 심볼을 링크할 수 있다.
...
- [First-stage init builds minimal filesystem](../../boot-and-runtime/init-service/first-stage-init-builds-minimal-filesystem-for-second-stage.md)
