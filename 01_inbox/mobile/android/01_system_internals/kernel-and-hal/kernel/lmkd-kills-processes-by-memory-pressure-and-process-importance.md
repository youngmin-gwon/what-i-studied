...
- `oom_score_adj` 기반 프로세스 우선순위 정책은 [Process priority는 memory reclaim 정책 입력이다](../../boot-and-runtime/system-server/process-priority-is-memory-reclaim-policy-input-not-app-state-truth.md)가 다룬다.
...
- [Process priority는 memory reclaim 정책 입력이다](../../boot-and-runtime/system-server/process-priority-is-memory-reclaim-policy-input-not-app-state-truth.md)
