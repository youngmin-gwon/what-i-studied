...
- BufferQueue에서의 producer/consumer 소유권 모델은 [BufferQueue는 producer와 consumer를 buffer ownership으로 분리한다](../../graphics-and-media/bufferqueue-ownership.md)가 다룬다.
- Android shared memory 진화 (ashmem→ION→DMA-BUF heaps)는 [Android shared memory는 ashmem, ION, DMA-BUF heaps로 역할이 분화됐다](android-shared-memory-evolution.md)가 다룬다.
...
- [BufferQueue는 producer와 consumer를 buffer ownership으로 분리한다](../../graphics-and-media/bufferqueue-ownership.md)
