...
- 이 노트는 lmkd 의 kill 결정 메커니즘을 다룬다. PSI 지표 자체의 해석은 [PSI는 free memory가 아니라 stall time을 측정한다](psi-pressure-stall-information.md)가 다룬다.
- `oom_score_adj` 기반 프로세스 우선순위 정책은 [Process priority는 memory reclaim 정책 입력이다](../../boot-and-runtime/system-server/process-priority-oom-score.md)가 다룬다.
...
- [Process priority는 memory reclaim 정책 입력이다](../../boot-and-runtime/system-server/process-priority-oom-score.md)
