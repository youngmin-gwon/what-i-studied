...
Android 커널은 [Binder IPC 커널 드라이버](../ipc-and-process/binder-ipc.md), [Low Memory Killer (LMK)](kernel/lmk-low-memory-killer.md), [WakeLocks 전력 제어](kernel/wake-locks.md), 및 [Ashmem / memfd 공유 메모리](kernel/ashmem.md) 를 표준 커널 확장 모듈로 장착하고 있다.
...
- **Android Kernel (모바일 전용으로 특수 튜닝된 레이싱 엔진)**:
  - 컴퓨터 과학의 [표준 Linux 커널 엔진](../../../../operating-systems/linux-kernel.md) 에 배터리 아끼는 터보 부스터([WakeLocks](kernel/wake-locks.md)), 메모리 좁을 때 자리를 비워주는 초스피드 요원([LMK](kernel/lmk-low-memory-killer.md)), 부서 간 빛의 속도로 연결하는 고속 통로([Binder IPC](../ipc-and-process/binder-ipc.md)), 및 공동 칠판([Ashmem](kernel/ashmem.md))을 부착한 안드로이드 스마트폰 전용 튜닝 커널.
...
2. **[Low Memory Killer (LMK / lmkd)](kernel/lmk-low-memory-killer.md)**:
   - 가용 RAM 부족 시 `oom_score_adj` 점수에 따라 백그라운드 프로세스를 단계별로 수거하는 선제적 메모리 관리기.
3. **[WakeLocks (`/sys/power/wake_lock`)](kernel/wake-locks.md)**:
   - 화면이 꺼진 후에도 CPU 가 Deep Sleep 에 들어가지 않도록 전력을 강제 유지하는 전력 관리 시스템.
4. **[Ashmem / memfd 익명 공유 메모리](kernel/ashmem.md)**:
...
- [Low Memory Killer (LMK)](kernel/lmk-low-memory-killer.md) - 안드로이드 LMK 메모리 회수기
- [WakeLocks 전력 유지](kernel/wake-locks.md) - 안드로이드 WakeLocks 전력 통제
- [Ashmem 공유 메모리](kernel/ashmem.md) - 안드로이드 Ashmem / memfd 공유 메모리
- [HAL (Hardware Abstraction Layer)](hal-native/hal.md) - 커널 드라이버 상부 HAL 레이어
