...
JNI string/array API 는 raw pointer 접근처럼 보여도 copy 또는 pin 이 될 수 있는 runtime contract 다. 포인터 주소가 managed object 의 실제 주소라고 가정하거나, release 없이 오래 보관하면 GC 와 memory behavior 를 깨뜨릴 수 있다.
...
Primitive array 접근에서 `Get<Type>ArrayElements` 는 구현에 따라 copy 또는 pin 을 반환할 수 있다. 단순 구간 복사는 `Get<Type>ArrayRegion` 과 `Set<Type>ArrayRegion` 이 release 누락 위험을 줄일 수 있다.
