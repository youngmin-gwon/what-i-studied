...
`GetMethodID`, `GetStaticMethodID`, `GetFieldID` 는 이름과 JNI signature 를 runtime metadata 로 해석해 ID 를 돌려준다. 반복 호출 경로에서는 lookup 비용보다 class reference lifetime 과 failure path 를 함께 고려해 캐시한다.

JNI 호출이 Java exception 을 발생시키면 native stack 이 C++ exception 처럼 자동 unwind 되지 않는다. 현재 thread 에 pending exception 상태가 남고, 대부분의 JNI 호출을 계속하면 안 된다.

`ExceptionCheck`, `ExceptionDescribe`, `ExceptionClear`, `Throw`, `ThrowNew` 는 "native 에서 예외를 던진다"보다 managed 경계로 돌아갈 pending state 를 제어하는 API 로 이해하는 편이 정확하다.
