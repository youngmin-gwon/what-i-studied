...
ION은 device/SoC별 heap을 통해 multimedia buffer allocation을 다루던 Android-specific allocator였다. 그러나 heap id/flag와 `/dev/ion` 단일 접근 모델은 표준화와 sepolicy 분리에 약했다.

Android 12의 GKI 2.0에서는 kernel 5.10 계열에서 ION allocator가 DMA-BUF heaps로 대체되는 방향을 갖는다. DMA-BUF heaps는 heap별 character device와 upstream ABI 안정성을 통해 vendor/device buffer allocation을 더 명확히 분리한다.
