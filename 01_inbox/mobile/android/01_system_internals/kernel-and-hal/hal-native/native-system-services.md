...
2. **ServiceManager Registration**: 실행된 네이티브 프로세스는 `defaultServiceManager()->addService()`를 호출하여 자사 [binder ipc](../../ipc-and-process/binder-ipc.md) 핸들을 `servicemanager` 에 등록한다. 클라이언트(Java Framework 또는 타 Native Process)는 서비스 이름을 통해 이 Binder 엔드포인트를 획득하여 통신한다.
...
- [Native service 디버깅은 init, Binder, VINTF, SELinux, tombstone을 분리한다](native-service-debugging.md)
