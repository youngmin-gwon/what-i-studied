...
- VINTF 호환성 선언은 [VINTF는 framework/vendor 호환성을 manifest와 matrix로 선언한다](vintf-manifest-compatibility.md)가 다룬다.
- Native system service와 Binder endpoint 등록은 [Native system service는 init이 띄우고 Binder로 발견되는 endpoint다](native-system-services.md)가 다룬다.
...
- [Native system service는 init이 띄우고 Binder로 발견되는 endpoint다](native-system-services.md)
