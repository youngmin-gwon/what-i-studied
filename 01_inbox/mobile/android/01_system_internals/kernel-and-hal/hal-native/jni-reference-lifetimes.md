...
- JNI 경계 자체(함수 등록, 타입 규약)는 [JNI는 managed runtime과 native code 사이의 명시적 호출 경계다](jni-native-boundary.md)가 다룬다.
- pending exception 처리는 [JNI method/field ID와 pending exception은 runtime state다](jni-runtime-state.md)가 다룬다.
