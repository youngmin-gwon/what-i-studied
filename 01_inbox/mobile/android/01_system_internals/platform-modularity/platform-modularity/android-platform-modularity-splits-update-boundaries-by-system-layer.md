...
Treble 과 GKI 는 더 낮은 층위의 경계다. **Treble**(`/system` framework 코드와 `/vendor` 제조사 하드웨어 구현을 버전 고정된 인터페이스로 분리해, 커널/드라이버를 안 건드리고도 framework 만 OTA할 수 있게 하는 경계 — 정식 정의는 [Treble separates system and vendor through stable interfaces](../../kernel-and-hal/hal-native/treble-separates-system-and-vendor-through-stable-interfaces.md) 참고)은 system image 와 vendor implementation 을 stable interface 로 분리하고, **GKI**(Generic Kernel Image — SoC/제조사와 무관하게 공통으로 배포되는 커널 코어. 정식 정의는 [GKI splits generic core from vendor modules](../../kernel-and-hal/kernel/gki-splits-generic-core-from-vendor-modules.md) 참고)는 common kernel 과 vendor kernel module 을 **KMI**(Kernel Module Interface — 커널 코어와 vendor 커널 모듈 사이의 심볼 호환성 경계) 경계로 분리한다.
...
관련 노트: [Mainline 경계](mainline-updates-selected-system-components-outside-normal-platform-releases.md), [Treble 정본](../../kernel-and-hal/hal-native/treble-separates-system-and-vendor-through-stable-interfaces.md), [GKI 정본](../../kernel-and-hal/kernel/gki-splits-generic-core-from-vendor-modules.md).
