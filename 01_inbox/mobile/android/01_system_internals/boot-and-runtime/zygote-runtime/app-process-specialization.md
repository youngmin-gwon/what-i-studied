...
앱 프로세스 생성 과정은 단순히 Zygote의 `fork()`로 끝나지 않으며, 고유 UID/GID 적용, SELinux 도메인 강등, Cgroup 바인딩, App-specific ClassLoader 로딩으로 고유 샌드박스 환경을 구체화하는 **Specialization** 단계를 거친 직후, `ActivityThread.main()`에서 [AMS](../../../04_system_services/service-lookup/activity-manager-service.md)로 Binder `attachApplication()`을 호출하여 비로소 Android 프레임워크 프로세스로 통합되는 메커니즘이다.
...
3. **[AMS](../../../04_system_services/service-lookup/activity-manager-service.md) Attach (`attachApplication`)**:
   - `ActivityThread` 인스턴스는 [AMS](../../../04_system_services/service-lookup/activity-manager-service.md)로 Binder IPC인 `attachApplication(mAppThread)`을 호출하여 자식 프로세스가 구동 완료되었음을 프레임워크에 알린다.
