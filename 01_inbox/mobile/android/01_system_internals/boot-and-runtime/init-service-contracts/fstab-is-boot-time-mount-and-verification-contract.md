...
date modified: 2026-08-03 17:23:36 +09:00
...
Android 의 fstab 은 어떤 block device 를 어디에 mount 할지뿐 아니라 `wait`, `slotselect`, `avb`, `logical`, `first_stage_mount`, `fileencryption`, `checkpoint` 같은 boot-time policy 를 함께 선언한다.

### 판단 기준

- `first_stage_mount` 는 second stage init 이 vendor/system rc 와 정책을 읽기 전에 필요한 파티션을 올린다.
- `slotselect` 는 A/B slot suffix 를 mount 대상 선택에 반영한다.
- `avb` 는 mount 경로와 Verified Boot 검증 경계를 연결한다.
- `/data` mount 는 file-based encryption, metadata encryption, checkpoint 와 함께 부팅 성공 여부에 영향을 준다.
