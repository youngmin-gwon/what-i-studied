---
title: "init service는 재시작 정책을 가진 supervised process다"
...
# init service는 재시작 정책을 가진 supervised process다
...
init service는 `service <name> <pathname> ...`로 선언된 native process이며, `init`이 시작, 중지, 재시작, class 제어, socket 생성, crash handling을 관리한다.

## 판단 기준

- long-running daemon은 service로 선언하고 init의 재시작 정책에 맡긴다.
- `oneshot`은 종료 후 자동 재시작하지 않을 작업에만 쓴다.
- `disabled` 서비스는 class start로 자동 시작되지 않고 명시적 start가 필요하다.
