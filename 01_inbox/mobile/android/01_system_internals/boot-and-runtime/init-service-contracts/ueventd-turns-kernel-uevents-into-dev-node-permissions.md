---
title: ueventd-turns-kernel-uevents-into-dev-node-permissions
...
date modified: 2026-08-03 17:23:47 +09:00
...
`ueventd` 는 kernel 이 보낸 device event 를 받아 `/dev` node 생성, owner/group, mode, SELinux label 을 적용한다. 앱이나 framework 가 하드웨어에 접근하는 권한은 이 dev node 경계에서 이미 한 번 결정된다.

### 왜 중요한가

camera, audio, binder, hwbinder, input device 같은 node 권한이 잘못되면 framework service 가 정상이어도 기기가 동작하지 않는다. 반대로 너무 넓은 권한은 HAL 또는 native daemon 의 보안 경계를 약하게 만든다.
