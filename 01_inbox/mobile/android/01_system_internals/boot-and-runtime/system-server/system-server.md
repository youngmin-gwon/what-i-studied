...
date modified: 2026-08-24 16:52:00 +09:00
...
- **App Framework 와 정책의 분리**: `Activity`, `Service`, `BroadcastReceiver` 등 앱 측 API 구문은 App Framework 정본 문서가 다루며, 이 묶음은 `system_server` 내부에서 이들 프로세스 및 생명주기를 강제 집행하는 정책에 집중한다.
- **Binder IPC 세부 분리**: Binder IPC 커널 드라이버, `Parcel`, `transact()` 멀티스레드 웅덩이는 IPC 정본으로 위임하고, 이 묶음은 시스템 서비스가 등록된 Binder Endpoint 라는 계약 사실만 연동한다.
- **메모리 회수 구현체 분리**: Linux Kernel 의 `PSI` (Pressure Stall Information) 및 `LMKD` 데몬 커널 모듈 세부는 Kernel/HAL 정본으로 이관하고, [AMS](../../../04_system_services/activity-manager-service.md)가 계산하여 건네는 `oom_score_adj` 입력 인터페이스에 집중한다.
