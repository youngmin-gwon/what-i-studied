...
1. **Shared Memory Read (mmap `/dev/__properties__`)**:
   - 속성 값 읽기(`SystemProperties.get()`) 시 [binder ipc](../../ipc-and-process/binder-ipc.md) 오버헤드를 없애기 위해, `init`은 Trie 트리가 구축된 Trie 데이터 구조체의 속성 영역을 `/dev/__properties__` 공유 메모리로 마운트한다.
...
- [init triggers는 event와 property 조건을 결합하는 실행 gate다](init-triggers.md)
