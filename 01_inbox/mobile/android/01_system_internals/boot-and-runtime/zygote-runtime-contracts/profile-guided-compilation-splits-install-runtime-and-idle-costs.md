...
Profile guided compilation은 앱 설치 시 모든 코드를 컴파일하는 방식과 실행 시 전부 JIT하는 방식 사이의 절충이다. 앱은 처음에는 해석 또는 JIT로 실행되고, runtime profile과 cloud/baseline profile을 바탕으로 idle compile 단계에서 자주 쓰는 경로를 AOT 컴파일할 수 있다.

## 실무 의미

- 설치 속도, 저장소 사용량, 첫 실행 성능은 서로 trade-off다.
- Play가 전달하는 dex metadata나 앱의 Baseline Profile은 초기 AOT 범위에 영향을 줄 수 있다.
- 사용 중 생성된 local profile은 이후 idle compile 또는 다음 실행 성능에 반영될 수 있다.
- cold start 성능을 보려면 profile 설치 상태와 compile filter를 고정해야 한다.
