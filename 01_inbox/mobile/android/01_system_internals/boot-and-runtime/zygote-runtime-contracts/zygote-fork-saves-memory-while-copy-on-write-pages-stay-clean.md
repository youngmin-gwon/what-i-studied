...
2. **Single-Threaded Constraint Before `fork()`**:
   - POSIX `fork()` 특성상 부모 프로세스의 다른 스레드가 뮤텍스(Mutex) 락을 쥐고 있는 상태에서 `fork()`를 수행하면 자식 프로세스에는 락을 해제해줄 스레드가 존재하지 않아 즉시 **[데드락(Deadlock)](01_inbox/operating-systems/deadlock.md)**(서로가 상대방이 들고 있는 자원을 기다리며 아무도 더 진행하지 못하는 영구 정지 상태)이 발생한다.
