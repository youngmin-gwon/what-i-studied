...
| :--- | :--- | :--- |
| **[Zygote는 framework 공통 상태를 preload한 뒤 앱 프로세스를 fork한다](zygote-preloads-framework-state-before-app-fork.md)** | `/system/etc/preloaded-classes` 및 `framework.jar` 자원 preloading, `ZygoteInit.main()` | `logcat \| grep Zygote`, `preloaded-classes` |
| **[Zygote fork의 메모리 이점은 copy-on-write가 유지될 때 생긴다](zygote-fork-saves-memory-while-copy-on-write-pages-stay-clean.md)** | `fork()` 페이지 테이블 복사, Clean Shared Page(OAT/VDEX) 무변경 유지 및 PSS/USS 절감 | `dumpsys meminfo`, `/proc/<pid>/smaps_rollup` |
| **[Zygote socket은 system_server가 앱 프로세스를 요청하는 factory interface다](zygote-socket-is-system-server-process-factory-interface.md)** | `/dev/socket/zygote` Unix Socket, `Process.start()`, `ZygoteServer` 커맨드 루프 | `ls -la /dev/socket/zygote`, `logcat -s Zygote` |
...
| **[ART는 DEX를 interpretation, JIT, AOT 조합으로 실행한다](art-runs-dex-with-interpretation-jit-and-aot.md)** | Interpreter -> JIT Compiler (Hot Methods) -> Profile-guided AOT (`dex2oat`) 3단계 티어링 | `dumpsys package <pkg>`, `art_dispatch` |
...
- **프로세스 생성과 컴포넌트 생명주기의 분리**: 프로세스 생성 및 Specialization은 Zygote/`ActivityThread` 부트스트랩 영역에서 처리하고, Activity/Service 자체의 생명주기 제어는 [system_server 계약](../system-server-contracts/system-server-contracts.md) 정본으로 이관한다.
...
관련 지도: [system_server와 ActivityManager 계약](../system-server-contracts/system-server-contracts.md), [Kernel contracts](../../kernel-and-hal/kernel-contracts/kernel-contracts.md)
