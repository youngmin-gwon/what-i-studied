...
1. **ANR Timeout Window 타임라인**:
   - **InputDispatchingTimeout**: 5초 (`InputDispatcher`에서 입력 이벤트에 대한 ACK 수신 실패).
   - **BroadcastQueue Timeout**: Foreground 10초, Background 60초.
   - **ServiceTimeout**: Foreground Service `onCreate`/`onStartCommand` 20초, Background Service 200초.
   - **ContentProviderTimeout**: 10초.
