...
tags: [android, system-internals, kernel, android-kernel, lmk, ashmem, binder, wakelocks]
aliases: [Android Kernel, 안드로이드 커널, Android Linux Kernel]
...
Android 커널은 [Binder IPC 커널 드라이버](binder-ipc.md), [Low Memory Killer (LMK)](low-memory-killer-lmk.md), [WakeLocks 전력 제어](wake-locks.md), 및 [Ashmem / memfd 공유 메모리](ashmem.md)를 표준 커널 확장 모듈로 장착하고 있다.
...
* **Android Kernel (모바일 전용으로 특수 튜닝된 레이싱 엔진)**:
  - 컴퓨터 과학의 [표준 Linux 커널 엔진](../../../operating-systems/linux-kernel.md)에 배터리 아끼는 터보 부스터([WakeLocks](wake-locks.md)), 메모리 좁을 때 자리를 비워주는 초스피드 요원([LMK](low-memory-killer-lmk.md)), 부서 간 빛의 속도로 연결하는 고속 통로([Binder IPC](binder-ipc.md)), 및 공동 칠판([Ashmem](ashmem.md))을 부착한 안드로이드 스마트폰 전용 튜닝 커널.
...
- [Low Memory Killer (LMK)](low-memory-killer-lmk.md) - 안드로이드 LMK 메모리 회수기
