---
title: "Wear OS 앱은 동반 휴대폰 앱과 독립적으로 실행될 수 있다"
...
워치 앱과 휴대폰 앱이 데이터를 주고받아야 한다면 Wearable Data Layer API(`DataClient`, `MessageClient`, `CapabilityClient`)를 사용한다. 이 API는 두 기기가 항상 연결되어 있다고 가정하지 않고, 연결이 끊긴 동안의 데이터는 동기화 큐에 쌓아뒀다가 재연결 시 전달하는 방식으로 동작한다. `CapabilityClient`로 상대 기기에 특정 앱이 설치되어 있는지, 어떤 기능을 지원하는지 확인할 수 있다.
