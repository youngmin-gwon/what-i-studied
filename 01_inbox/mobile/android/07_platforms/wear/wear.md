...
1. [Wear OS 앱은 동반 휴대폰 앱과 독립적으로 실행될 수 있다](standalone-wear-os-apps.md) 에서 워치 단독 실행과 동반 앱 통신의 관계를 본다.
2. [Ambient mode는 절전 화면에서 앱 화면을 유지하는 별도 lifecycle이다](ambient-mode-lifecycle.md) 에서 일반 lifecycle 과 다른 점을 본다.
3. [Tile과 Complication은 워치페이스/런처에 데이터를 노출하는 별도 표면이다](tiles-and-complications-surfaces.md) 에서 앱 밖에서 데이터를 보여주는 경로를 본다.
...
- [Wear OS 앱은 동반 휴대폰 앱과 독립적으로 실행될 수 있다](standalone-wear-os-apps.md)
- [Ambient mode는 절전 화면에서 앱 화면을 유지하는 별도 lifecycle이다](ambient-mode-lifecycle.md)
- [Tile과 Complication은 워치페이스/런처에 데이터를 노출하는 별도 표면이다](tiles-and-complications-surfaces.md)
