...
- 이 노트는 Tile/Complication 이 메인 앱과 분리된 표면이라는 사실을 다룬다. 메인 앱 화면 자체의 절전 상태 처리는 [Ambient mode는 절전 화면에서 앱 화면을 유지하는 별도 lifecycle이다](./ambient-mode-is-a-separate-lifecycle-for-always-on-screens.md) 가 다룬다.
