...
- 이 노트는 워치 - 휴대폰 앱 간 독립성과 통신 모델을 다룬다. 화면이 꺼진 것처럼 보이는 절전 상태에서의 UI 유지는 [Ambient mode는 절전 화면에서 앱 화면을 유지하는 별도 lifecycle이다](ambient-mode-lifecycle.md) 가 다룬다.
