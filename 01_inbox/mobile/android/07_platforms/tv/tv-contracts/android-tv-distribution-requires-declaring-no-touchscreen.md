...
aliases: []
date modified: 2026-08-03 18:16:10 +09:00
...
Google Play 는 기기 호환성을 매니페스트의 `<uses-feature>` 선언으로 판단한다. 대부분의 안드로이드 앱은 터치스크린을 암묵적으로 필요로 하는 것으로 취급되므로, TV 처럼 터치스크린이 없는 기기에는 기본적으로 노출되지 않는다. `leanback` 기능을 필수로 선언하면 Play 는 이 앱을 TV 용 앱 카테고리로 분류하고, TV 기기의 홈 화면 런처(Leanback launcher)에 표시할 배너 이미지(`android:banner`)도 별도로 요구한다.
