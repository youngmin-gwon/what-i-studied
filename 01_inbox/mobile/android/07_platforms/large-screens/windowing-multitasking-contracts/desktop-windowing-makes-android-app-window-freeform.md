...
### Freeform Window 설정 및 Manifest 제약
...
```bash
# 데스크톱 자유 창 모드 (Windowing Mode 5: FREEFORM) 확인
adb shell dumpsys activity displays | grep -E "mWindowingMode|freeform"

# 창 크기 변경에 따른 Activity Configuration 변경 로그 관측
adb logcat -v threadtime | grep -E "wm_on_configuration_changed|WM-ActivityTask"
```

### 관련 문서

- [창 크기 클래스는 기기 종류가 아니라 앱 창을 분류한다](../large-screen-contracts/window-size-class-classifies-app-window-not-device-type.md)
- [키보드, 포인터, 스타일러스는 큰 화면의 기본 입력이다](../large-screen-contracts/keyboard-pointer-and-stylus-are-primary-large-screen-inputs.md)
