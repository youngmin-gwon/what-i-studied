...
### `FoldingFeature` 감지 메커니즘 및 수명 연동
                        ...
                        if (foldingFeature != null && foldingFeature.isSeparating) {
                            when (foldingFeature.orientation) {
                                FoldingFeature.Orientation.HORIZONTAL -> {
                                    // Tabletop Posture (Control Bottom Pane, Video Top Pane)
                                    enterTabletopMode(foldingFeature.bounds)
                                }
                                FoldingFeature.Orientation.VERTICAL -> {
                                    // Book Posture (Left-Right Split Pane)
                                    enterBookMode(foldingFeature.bounds)
                                }
                            }
                        }
                    }
...
```mermaid
graph TD
    A["WindowInfoTracker.windowLayoutInfo() Flow"] --> B{"displayFeatures has FoldingFeature?"}
    B -- "No" --> C["Standard Flat Layout"]
    B -- "Yes" --> D{"foldingFeature.isSeparating?"}
    D -- "False (Flat Hinge)" --> C
    D -- "True (HALF_OPENED)" --> E{"Orientation"}
    E -- "HORIZONTAL" --> F["Tabletop Mode (Top Media, Bottom Controls)"]
    E -- "VERTICAL" --> G["Book Mode (Two-page Book View)"]
```
...
```bash
# 폴더블 에뮬레이터에서 posture 상태 시뮬레이션
adb shell wm posture half-opened
adb shell wm posture open
adb shell wm posture closed

# 윈도우 레이아웃 display features 디버깅
adb shell dumpsys window displays | grep -i "FoldingFeature"
```

### 관련 문서

- [창 크기 클래스는 기기 종류가 아니라 앱 창을 분류한다](./window-size-class-classifies-app-window-not-device-type.md)
- [적응형 앱 준비도는 창, posture, 입력 테스트로 판단한다](./adaptive-app-readiness-requires-window-posture-input-testing.md)
