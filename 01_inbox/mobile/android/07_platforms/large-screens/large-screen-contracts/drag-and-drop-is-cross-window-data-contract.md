...
- 외부 앱에서 들어온 URI는 drop 직전에 `requestDragAndDropPermissions()`로 임시 접근을 얻고, 사용이 끝나면 반환 객체의 `release()`로 해제한다. 다른 activity로 넘기면 `ClipData`와 URI grant를 명시한다.
...
검증일: 2026-08-03. 앱 간 URI drag의 권한 플래그와 접근 수명은 multi-window drag 문서 기준이다.
