...
- Spatialized component를 쓸 수 없는 상태에는 제품이 명시적으로 2D 대응 UI를 유지해야 한다.
...
공간화는 Full Space에서만 지원된다. Home Space에서는 앱이 다른 앱과 함께 2D 패널로 실행되므로, `LocalSession`에서 요청한 space 전환 결과와 현재 capability를 확인한 뒤 공간 UI를 노출한다. Compose for XR API가 존재한다는 사실만으로 현재 공간화가 가능하다고 판단하지 않는다.
