...
- 현재 환경에서 가능한 기능을 runtime capability로 확인한 뒤 UI를 선택한다.
- 공간 기능이 없을 때 2D fallback이 자연스럽게 남아야 한다.
- capability 변화는 일회성 초기화 값이 아니라 UI state 입력으로 취급한다.
...
공식 문서: [Jetpack Compose for XR release notes](https://developer.android.com/jetpack/androidx/releases/xr-compose)
