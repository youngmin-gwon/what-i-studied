---
title: xr-multimodal-input-gaze-hands
...
- 이 노트는 입력 소스 설계와 처리 원칙을 다룬다. 공간 capability 의 런타임 확인 방법은 [XR 앱은 공간 capability 를 실행 중에 확인해야 한다](xr-spatial-capability-checks.md) 가 다룬다.
- 입력 지연(latency)이 멀미에 미치는 영향은 [XR 품질은 성능, 편안함, 안전을 기능 요구사항으로 포함한다](xr-comfort-safety-performance.md) 가 다룬다.
...
- [XR 앱은 공간 capability 를 실행 중에 확인해야 한다](xr-spatial-capability-checks.md)
