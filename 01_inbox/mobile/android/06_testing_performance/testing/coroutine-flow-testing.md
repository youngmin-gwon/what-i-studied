...
- **`UnconfinedTestDispatcher`**:
  - 자식 코루틴을 첫번째 중단점(Suspension point)까지 즉시 동기적(Eagerly)으로 실행한다. 코루틴 수집([stateflow](../../02_app_framework/async-flow/flow-state/stateflow-and-sharedflow.md) assertion) 시 `runCurrent()` 호출 횟수를 줄여주나 실행 순서 제어권은 낮다.
