---
title: pipeline-sharding-should-balance-by-historical-duration-not-test-count
...
관련 노트: [CI 는 Firebase Test Lab 같은 클라우드 디바이스 매트릭스에서 테스트를 실행하고 로컬 에뮬레이터 매트릭스와는 다른 계약을 가진다](./firebase-test-lab-runs-ci-tests-on-a-cloud-device-matrix-not-local-emulators.md)
...
이 노트는 "왜 시간 기준으로 나눠야 하는가" 와 "어떻게 나누는가" 만 다룬다. 클라우드 디바이스 매트릭스 자체의 구성(기기 조합, locale/orientation)은 [CI는 Firebase Test Lab 같은 클라우드 디바이스 매트릭스에서 테스트를 실행하고 로컬 에뮬레이터 매트릭스와는 다른 계약을 가진다](./firebase-test-lab-runs-ci-tests-on-a-cloud-device-matrix-not-local-emulators.md) 를 본다. flaky test 자체의 격리/quarantine 정책은 [회귀와 flaky 테스트는 릴리즈 게이트의 신뢰도를 낮춘다](./regression-and-flaky-tests-are-release-gate-risks.md) 를 본다.
