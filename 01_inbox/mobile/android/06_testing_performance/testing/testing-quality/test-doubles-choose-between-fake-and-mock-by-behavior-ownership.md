---
title: test-doubles-choose-between-fake-and-mock-by-behavior-ownership
...
이 노트는 test double 선택 기준만 다룬다. 어떤 테스트 레이어(unit/integration/UI/E2E)에서 test double 을 쓸지는 [테스트 레이어는 피드백 비용으로 선택한다](./test-layer-is-chosen-by-feedback-cost-and-risk.md)가 다룬다. dispatcher/코루틴 자체를 통제하는 방법은 [Coroutine 과 Flow 테스트는 dispatcher 와 virtual time 을 통제해야 한다](../coroutine-flow-tests-control-dispatchers-and-virtual-time.md)가 다룬다.
...
관련 노트: [테스트 레이어는 피드백 비용으로 선택한다](./test-layer-is-chosen-by-feedback-cost-and-risk.md), [회귀와 flaky 테스트는 릴리즈 게이트의 신뢰도를 낮춘다](./regression-and-flaky-tests-are-release-gate-risks.md)
