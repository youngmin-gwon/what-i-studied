---
title: "Coroutine과 Flow 테스트는 dispatcher와 virtual time을 통제해야 한다"
...
Testing/performance 노트는 빠른 피드백, release gate, 진단 trace, 반복 가능한 benchmark를 분리해 선택하는 기준으로 읽는다.

## 경계

단일 측정값보다 재현 조건, device state, test layer, failure cost를 함께 고정한다.
