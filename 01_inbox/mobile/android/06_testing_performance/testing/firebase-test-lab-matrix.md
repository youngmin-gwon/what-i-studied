---
title: firebase-test-lab-matrix
...
이 노트는 "클라우드 매트릭스와 로컬 매트릭스가 왜 다른 것을 검증하는가"만 다룬다. 파이프라인이 각 매트릭스 셀에 테스트를 어떻게 분배하는지는 [파이프라인 sharding 은 테스트 개수가 아니라 과거 실행 시간 기준으로 분배해야 한다](test-pipeline-sharding.md) 를 본다. CI/CD 파이프라인 단계 구성 자체(체크아웃 → lint → test → 서명 → 배포)는 이 노트의 범위가 아니다.
