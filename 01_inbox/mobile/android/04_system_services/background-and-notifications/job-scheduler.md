---
title: job-scheduler
tags: [android, background, deferrable, job-scheduler, system-services]
aliases: [JobScheduler, JobSchedulerService, 스마트 에코 스케줄러]
date modified: 2026-08-06 19:00:32 +09:00
...
앱이 개별적으로 배터리와 네트워크를 소비하지 않고, OS 가 중앙에서 수많은 앱의 백그라운드 작업을 묶어서 한 번에 처리함으로써 기기의 배터리 효율과 시스템 자원(CPU/RAM) 소비를 극대화한다.
...
- **`JobInfo` (세탁 예약 조건 카드)**:
  - "이 빨래는 꼭 Wi-Fi 가 켜지고 충전 케이블이 꽂혔을 때만 돌려주세요"라고 적어둔 작업 조건 정의서.
- **`JobService` (실제 세탁 코스 수행자)**:
  - 조건을 만족하면 시스템에 의해 깨어나 실제 작업을 수행하는 앱의 백그라운드 실행 컴포넌트.
- **`Batching` (모아서 한 번에 세탁하기)**:
  - 10 개 앱의 예약 작업을 묶어서 배터리/라디오 모뎀을 한 번만 깨워 처리하는 연비 최적화 기법.
...
### 2. JobScheduler 의 제약 조건 (Constraints) 및 작동 원리
...
- Android 의 **Doze 모드(휴면 모드)** 및 **App Standby** 상태와 연동되어 지연 가능한 백그라운드 실행을 자동으로 조정한다.
...
### 4. Jetpack WorkManager 와의 관계

- **WorkManager 의 기본 엔진**:
  - Android Jetpack 의 **WorkManager**는 하위 호환성을 보장하는 백그라운드 작업 표준 라이브러리로, API 23 이상 기기에서는 내부적으로 **JobScheduler**를 기본 엔진으로 사용하여 백그라운드 작업을 실행한다.
...
- [system_server](../../01_system_internals/boot-and-runtime/system-server/system-server.md) - JobSchedulerService 가 상주하는 메인 시스템 프로세스
- [ServiceManager](../service-lookup/service-manager.md) - JobSchedulerService 의 Binder 참조를 관리하는 전역 디렉토리
- [WorkManager 레퍼런스](work-manager.md) - 보장된 지연 가능 백그라운드 작업 표준 래퍼 라이브러리
