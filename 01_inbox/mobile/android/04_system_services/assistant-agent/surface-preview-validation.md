---
title: surface-preview-validation
...
## App Actions 와 AppFunctions 도입은 preview 와 호출 표면을 검증해야 한다
...
- [ ] 기존 BII 가 있는지 먼저 확인했다.
- [ ] BII 에 맞지 않는 기능만 custom intent 를 검토했다.
- [ ] 함수로 노출할 최소 작업 단위를 정의했다.
- [ ] UI 가 필요한 작업과 UI 없이 가능한 작업을 구분했다.

### App Actions

- [ ] `shortcuts.xml` 을 `res/xml` 에 두었다.
- [ ] 각 `capability` 에 fulfillment intent 를 정의했다.
- [ ] BII parameter 를 명시적 extra 또는 URL parameter 에 정확히 매핑했다.
- [ ] 모호한 엔터티를 검색·선택 화면으로 보낼 수 있다.
- [ ] parameter 가 없는 질의를 위한 fallback fulfillment 를 검토했다.
- [ ] 지원 언어와 지역별 동작을 확인했다.
- [ ] Assistant test tool 과 실제 기기에서 질의를 테스트했다.
...
- [ ] KDoc, 함수명, parameter 명, 결과 타입을 에이전트 관점에서 검토했다.
- [ ] 함수가 실행 가능한 상태인지와 실패 이유를 반환한다.
- [ ] 지원하지 않는 기기에서는 기능을 숨기거나 안전한 fallback 을 제공한다.
...
App Actions 와 AppFunctions 의 API·권한·지원 범위는 플랫폼 및 preview 상태에 따라 변할 수 있다.
