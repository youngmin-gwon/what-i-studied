---
title: bluetooth-gatt-state-machine
...
- 이 노트는 연결 상태 관리까지만 다룬다. 연결에 필요한 런타임 권한 조건은 [Android 12+ Bluetooth 런타임 권한은 조건부로만 위치 권한을 대체한다](bluetooth-runtime-permissions.md)가 다룬다.
- 연결 대상 기기를 찾는 스캔 단계는 [BLE 백그라운드 스캔은 배터리 제약을 받으며 ScanFilter가 필요하다](ble-background-scanning.md)가 다룬다.
