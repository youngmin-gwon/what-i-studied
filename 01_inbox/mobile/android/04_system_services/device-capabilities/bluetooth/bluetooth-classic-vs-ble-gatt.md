---
title: bluetooth-classic-vs-ble-gatt
...
- 이 노트는 두 연결 모델의 API 차이까지만 다룬다. BLE GATT 연결 이후의 콜백 기반 상태 관리는 [BluetoothGatt 콜백 기반 연결은 명시적 상태 머신이 필요하다](bluetooth-gatt-state-machine.md)가 다룬다.
- Android 12+ 권한 선언 차이는 [Android 12+ Bluetooth 런타임 권한은 조건부로만 위치 권한을 대체한다](bluetooth-runtime-permissions.md)가 다룬다.
