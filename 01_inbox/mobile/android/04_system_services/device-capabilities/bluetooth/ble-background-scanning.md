...
- 이 노트는 스캔 단계의 배터리/백그라운드 제약과 필터링까지 다룬다. 스캔으로 찾은 기기에 실제로 연결한 뒤의 상태 관리는 [BluetoothGatt 콜백 기반 연결은 명시적 상태 머신이 필요하다](bluetooth-gatt-state-machine.md)가 다룬다.
- 스캔에 필요한 `BLUETOOTH_SCAN` 권한과 위치 권한 대체 조건은 [Android 12+ Bluetooth 런타임 권한은 조건부로만 위치 권한을 대체한다](bluetooth-runtime-permissions.md)가 다룬다.
