...
- 이 노트는 재생 상태를 외부에 노출하는 계약까지 다룬다. 여러 앱 간 오디오 재생 자체의 조정은 [AudioManager는 포커스 요청으로 여러 앱의 동시 재생을 조정한다](./audiomanager-arbitrates-concurrent-playback-through-focus-requests.md)가 다룬다.
