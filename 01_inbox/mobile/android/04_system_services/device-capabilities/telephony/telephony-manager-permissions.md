---
title: telephony-manager-permissions
...
- 이 노트는 permission 세분화까지 다룬다. 멀티 SIM 환경에서 어떤 SIM의 정보를 조회할지는 [SubscriptionManager는 멀티 SIM에서 논리적 구독과 물리 슬롯을 분리한다](subscription-manager-slots.md)가 다룬다.
- 통신사 서명 기반의 특권적 접근(일반 permission 체계 밖)은 [Carrier privilege는 런타임 권한 없이 통신사 서명 인증서로 부여된다](carrier-privilege-certificates.md)가 다룬다.
