...
- 이 노트는 권한 요청·승인 모델까지만 다룬다. Health Connect 자체가 온디바이스 저장소이지 클라우드 서비스가 아니라는 위치 모델은 [Health Connect는 클라우드 동기화가 아니라 앱 간 공유 온디바이스 저장소다](./health-connect-is-a-shared-on-device-store-not-a-cloud-sync-service.md)가 다룬다.
