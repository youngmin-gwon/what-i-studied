---
title: on-device-ai-feature-availability
...
- 이 노트는 가용성 확인 절차까지만 다룬다. 온디바이스와 클라우드 추론의 차이는 [온디바이스 추론은 클라우드 추론이 필요로 하는 네트워크 왕복을 건너뛴다](on-device-inference-low-latency.md)가, 모델을 시스템이 공유 관리하는 계약은 [AICore는 Gemini Nano를 앱마다 번들되지 않는 공유 시스템 모델로 관리한다](aicore-gemini-nano.md)가 다룬다.
