...
이 지도는 앱 아이콘 길게 누르기로 노출되는 App Shortcuts를 소유권과 lifecycle, 개수/rate limit 두 가지 계약으로 나눈다. static/dynamic/pinned shortcut은 이름은 비슷하지만 선언 위치, 갱신 주체, 삭제 가능 여부가 모두 다르며, `ShortcutManager`는 이 셋을 하나의 카운트 상한과 rate limit 아래 관리한다.

### 읽는 순서

1. [static/dynamic/pinned shortcut은 소유권과 lifecycle이 다르다](./static-dynamic-and-pinned-shortcuts-have-different-ownership-and-lifecycle.md)에서 세 종류가 각각 누구에 의해 만들어지고 누가 지울 수 있는지 본다.
...
- App Widget(`AppWidgetProvider`, `RemoteViews`)은 별도 계약이며 이 지도가 다루지 않는다. shortcut은 "탭하면 앱의 특정 화면/동작으로 진입하는 진입점"이고, widget은 "홈 화면에서 지속적으로 콘텐츠를 보여주는 표면"이라는 점에서 목적이 다르다.
