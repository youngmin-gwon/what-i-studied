...
2. [SpeechRecognizer는 RECORD_AUDIO 권한을 전제로 하는 콜백 기반 비동기 계약이다](./speechrecognizer-is-a-callback-based-contract-gated-by-record-audio-permission.md)에서 권한 전제조건과 `RecognitionListener` 등록 순서를 본다.
...
- Assistant 질의를 앱 fulfillment로 연결하는 의미 해석과 실행 책임은 [Assistant와 에이전트 통합 계약](../../agents-and-assistant/assistant-agent-contracts/assistant-agent-contracts.md)이 담당하며, 이 지도는 그 위에서 쓰이는 순수 음성 합성/인식 API 계약만 다룬다.
- 마이크 하드웨어 자체의 오디오 포커스 조정은 [미디어/오디오/카메라 시스템 서비스 접근 계약](../media-audio-camera-contracts/media-audio-camera-contracts.md)이 다루며, 이 지도는 `SpeechRecognizer`가 오디오 입력을 어떻게 소비하는지의 세부를 다루지 않는다.
...
- [SpeechRecognizer는 RECORD_AUDIO 권한을 전제로 하는 콜백 기반 비동기 계약이다](./speechrecognizer-is-a-callback-based-contract-gated-by-record-audio-permission.md)
