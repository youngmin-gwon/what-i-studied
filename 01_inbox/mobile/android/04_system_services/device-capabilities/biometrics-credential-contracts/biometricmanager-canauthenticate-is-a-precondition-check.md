---
title: "BiometricManager.canAuthenticate는 실행 전에 확인해야 하는 사전 조건이다"
...
- 이 노트는 실행 전 사전 조건 확인까지 다룬다. 인증 성공 후 키 사용 승인 메커니즘은 [BiometricPrompt는 인증 UI와 키 사용 승인을 함께 처리한다](01_inbox/mobile/android/04_system_services/device-capabilities/biometrics-credential-contracts/biometricprompt-couples-authentication-ui-with-key-authorization.md)가 다룬다.
- 로그인 자격 증명 자체의 통합(비밀번호, 패스키)은 [CredentialManager는 비밀번호/패스키/연동 로그인을 하나의 API로 통합한다](01_inbox/mobile/android/04_system_services/device-capabilities/biometrics-credential-contracts/credentialmanager-unifies-password-passkey-and-federated-sign-in.md)가 다룬다.
