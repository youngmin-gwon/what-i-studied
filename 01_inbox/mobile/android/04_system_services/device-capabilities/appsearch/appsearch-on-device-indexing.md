---
title: appsearch-on-device-indexing
...
- 이 노트는 저장소 선택과 System UI 노출 계약만 다룬다. 스키마 변경 시 마이그레이션 계약은 [Document 스키마 변경은 명시적 마이그레이션이 없으면 호환되지 않는 데이터를 삭제한다](appsearch-schema-migrations.md)가 다룬다.
