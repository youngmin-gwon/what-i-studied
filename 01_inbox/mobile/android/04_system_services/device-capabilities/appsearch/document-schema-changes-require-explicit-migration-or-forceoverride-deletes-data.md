...
- 이 노트는 스키마 버전 비교와 마이그레이션 계약만 다룬다. 저장소 선택과 System UI 노출 계약은 [AppSearch는 클라우드 검색 엔진이 아니라 온디바이스 검색 색인이다](./appsearch-is-an-on-device-search-index-not-a-cloud-search-engine.md)가 다룬다.
