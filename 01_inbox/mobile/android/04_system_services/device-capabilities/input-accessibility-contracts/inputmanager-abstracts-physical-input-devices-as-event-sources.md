...
- 이 노트는 입력 장치 추상화까지 다룬다. 이 이벤트를 가로채 다른 앱을 조작하는 특권 서비스는 [AccessibilityService는 다른 앱의 UI 이벤트를 관찰하고 조작할 수 있는 특권 서비스다](./accessibilityservice-observes-and-acts-on-other-apps-ui.md)가 다룬다.
- 텍스트 입력을 위한 소프트 키보드 자체의 계약은 [InputMethodService는 AccessibilityService와 다른 별도의 입력 계약이다](./inputmethodservice-is-a-separate-contract-from-accessibilityservice.md)가 다룬다.
