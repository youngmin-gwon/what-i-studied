---
title: "센서 좌표계는 화면 방향이 아니라 기기 고정 좌표계다"
...
가속도계, 자이로스코프 같은 motion 센서의 좌표축(X/Y/Z)은 기기가 기본 방향(natural orientation)일 때를 기준으로 고정되어 있다. 사용자가 화면을 회전시켜 UI가 가로/세로로 바뀌어도 센서 좌표축 자체는 회전하지 않는다.
...
- 화면 방향에 따라 동작이 달라져야 하는 기능(예: 수평계, 게임 컨트롤)은 `SensorManager.remapCoordinateSystem()`으로 현재 디스플레이 회전에 맞춰 좌표축을 변환한 뒤 사용한다.
