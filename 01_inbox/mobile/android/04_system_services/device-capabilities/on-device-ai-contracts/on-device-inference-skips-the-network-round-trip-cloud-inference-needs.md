...
반대로 클라우드 추론(Firebase AI Logic을 통한 Gemini 등)은 요청을 네트워크로 보내고 응답을 받는다.
...
- 이 노트는 추론 위치에 따른 차이까지만 다룬다. 온디바이스 모델을 앱마다 번들하지 않고 시스템이 공유 관리하는 AICore의 계약은 [AICore는 Gemini Nano를 앱마다 번들되지 않는 공유 시스템 모델로 관리한다](./aicore-manages-gemini-nano-as-a-shared-system-model-not-a-bundled-asset.md)가 다룬다.
...
Android Studio Network Profiler로 추론 호출을 관찰하면, 온디바이스 경로는 호출 시점에 네트워크 트래픽이 발생하지 않는 반면 클라우드 경로는 HTTPS 요청/응답이 기록된다. 이 차이로 "이 기능이 실제로 온디바이스로 도는지"를 코드를 보지 않고도 검증할 수 있다.
