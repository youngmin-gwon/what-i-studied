...
tags: ["android", "build-cache", "ci-cd", "performance"]
aliases: ["빌드 매트릭스와 원격 캐시는 함께 CI 매트릭스 시간을 줄인다"]
date modified: 2026-08-31 19:01:08 +09:00
...
2. **Remote Cache Pull/Push**:
   - PR 빌드 러너: Remote Cache 에서 `Pull` 전용으로 다운로드하여 캐시 히트(Cache FROM-CACHE)를 누림.
   - Main 브랜치 빌드 러너: 검증이 완료된 태스크 결과를 Remote Cache 로 `Push` 하여 캐시 업데이트.
