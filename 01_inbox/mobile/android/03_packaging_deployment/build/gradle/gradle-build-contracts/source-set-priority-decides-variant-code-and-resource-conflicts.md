...
aliases: []
date modified: 2026-08-03 18:12:36 +09:00
...
Source set 은 특정 빌드 대상에 함께 들어갈 Kotlin/Java 코드, 리소스, manifest 의 묶음이다.

기본 `main` 은 모든 변형의 공통 입력이고, `debug`, flavor, variant 전용 source set 은 차이를 표현한다.
...
공통 코드와 리소스는 `main` 에 두고, 변형별 차이만 더 구체적인 source set 으로 내린다.
...
각 source set 의 manifest 는 변형 구성에 따라 병합된다.

manifest 값을 덮어써야 할 때는 병합 규칙과 `tools:replace` 같은 명시적 의도를 함께 검토한다.

파일이 어느 variant 에 들어가는지는 directory 이름과 실제 선택 변형을 함께 확인한다.

### 명시적 경로 설정

표준 구조를 유지하는 것이 우선이며, 필요한 경우 `sourceSets` 블록으로 경로를 매핑할 수 있다.

경로를 바꾸면 IDE, 테스트, lint, CI 의 기대 경로도 함께 점검해야 한다.

### 실무 점검

- 같은 클래스가 `main` 과 변형 source set 에 중복되지 않는가?
- flavor 조합 전용 파일이 예상 variant 에만 포함되는가?
- 리소스 대체가 이름 충돌로 조용히 발생하지 않는가?
- 테스트 source set 과 앱 source set 의 역할이 분리되어 있는가?
