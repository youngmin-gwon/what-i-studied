...
tags: ["android", "sourceset", "agp", "gradle"]
...
- **코드 파일 (.kt / .java)**: 동일한 클래스 네임스페이스가 2개 이상 소스셋에 중복 정의되면 컴파일 타임 `Duplicate Class Error` 가 발생하므로 분리 관리해야 한다.
...
AGP의 리소스 병합 과정을 리소스 리포트 파일로 분석할 수 있다:
