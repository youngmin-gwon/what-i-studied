...
AGP는 Java/Kotlin 컴파일러 태스크 외에도 `aapt2` (리소스 컴파일 및 패키징), `R8/D8` (자바 바이트코드를 DEX 코드로 변환 및 축소), `Manifest Merger` (라이브러리 매니페스트 통합), 그리고 `apksigner/zipalign` (산출물 패키징 및 서명) 태스크를 Gradle Directed Acyclic Graph (DAG)에 연결한다.
