...
tags: ["android", "compiler", "kotlin", "serialization"]
aliases: ["kotlinx.serialization은 컴파일러 플러그인과 런타임 포맷이 모두 필요하다"]
date modified: 2026-08-26 17:48:05 +09:00
date created: 2026-07-31 17:52:17 +09:00
created: 2026-07-31 17:52:17 +09:00
updated: 2026-08-05 16:15:00 +09:00
---

## kotlinx.serialization 은 컴파일러 플러그인과 런타임 포맷이 모두 필요하다
...
Gson 이나 Jackson 같은 묵직한 Java 리플렉션(Reflection) 기반 라이브러리와 달리, `kotlinx.serialization` 은 **컴파일 타임(Compile-Time) 직렬화 코드 생성**을 채택한다.
...
2. **리플렉션 Zero(0)**: 컴파일러가 직접 필드 이터레이션 바이트코드를 합성하므로 R8 난독화 시 규칙이 없어도 런타임 crash 가 발생하지 않으며, 성능 및 메모리 효율성이 월등히 뛰어나다.
3. **두 축의 필수 결합**:
...
관련 노트: [KSP는 Kotlin 퍼스트 코드 생성이며 KAPT는 유지보수 모드다](ksp-code-generation.md), [의존성 및 CI 계약](gradle-build.md)
