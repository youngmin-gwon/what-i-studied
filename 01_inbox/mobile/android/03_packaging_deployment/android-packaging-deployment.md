...
date modified: 2026-08-24 17:47:23 +09:00
...
Android 패키징 및 배포 시스템은 다음 4 대 핵심 하위 시스템 파이프라인으로 구성된다:
...
1. **[Gradle 빌드 및 의존성 아키텍처](build/gradle/gradle-build.md)**: AGP, `defaultConfig`, Build Variant 매트릭스, Version Catalog(`libs.versions.toml`), 의존성 해소, KSP/Compose 컴파일러 플러그인, SourceSet 우선순위, Convention Plugin, `signingConfigs`.
2. **[Android CI/CD](build/ci-cd/ci-cd.md)**: Fastlane 오케스트레이션, CI 자격증명 보안, Remote Cache & Build Matrix, 파이프라인 실패 시그널.
3. **[R8와 Gradle 빌드 최적화 계약](optimization/build-optimization.md)**: R8 수축/최적화/난독화, Resource Shrinker, ProGuard Keep Rules, R8 Full Mode, Incremental/Build/Configuration Cache.
4. **[Play 릴리스와 배포 계약](distribution/release/release-distribution.md)**: AAB vs APK, Play App Signing, 업그레이드 호환성, 테스트 트랙, Staged Rollout, In-App Update/Review API.
5. **[Play Delivery 계약](distribution/play-delivery/play-delivery.md)**: Play Feature Delivery(PFD), Dynamic Feature Module(DFM), Play Asset Delivery(PAD), SplitInstallManager.
6. **[Google Play Billing 계약](distribution/billing/billing.md)**: 2026-08-06 기준 최신 Play Billing Library 9.1.0, 상품·지역·등록 프로그램별 결제 경로 판정, 서버 측 Purchase Token 검증, 구매 완료 후 3 일 이내 Acknowledge/Consume, 구독 라이프사이클. 라이브러리 버전과 정책은 출시 시점에 다시 확인한다.

### 추천 탐색 가이드
- **초급/개념 습득**: `Gradle 빌드 및 의존성 아키텍처` $\rightarrow$ `Play 릴리스와 배포 계약`
