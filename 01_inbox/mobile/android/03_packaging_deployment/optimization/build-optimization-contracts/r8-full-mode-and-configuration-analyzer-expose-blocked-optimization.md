...
- **R8 Full Mode**: ProGuard 호환성 제약을 제거하고 최적화를 극대화한다. 사용되지 않는 인자 제거, 인터페이스 인라이닝, 더 공격적인 클래스 병합(Class Merging)을 수행한다. AGP 8.0부터 Full Mode가 기본값이므로 `android.enableR8.fullMode` 플래그를 켤 필요가 없다. 이 플래그는 이제 반대로 `android.enableR8.fullMode=false`를 지정해 Compatibility Mode로 되돌리는 opt-out 용도로만 남아 있다.
...
```properties
# gradle.properties
# AGP 8.0+에서는 Full Mode가 기본값이므로 별도 설정이 필요 없다.
# 옛 Compatibility Mode로 되돌리려는 경우에만 아래 줄을 추가한다.
