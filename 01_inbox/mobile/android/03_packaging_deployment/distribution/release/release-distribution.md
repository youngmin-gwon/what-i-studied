...
tags: ["android", "android/packaging-deployment", "distribution", "release"]
aliases: ["릴리스 배포 계약"]
date modified: 2026-08-31 19:02:27 +09:00
...
1. **AAB 배포 규약**: 개발자는 모든 리소스와 DEX 가 포함된 `.aab` 아티팩트를 제출하고, Google Play 는 사용자 기기의 ABI(arm64-v8a 등), Screen Density(xhdpi 등), 언어 설정에 맞게 최적화된 **Split APK** 세트를 생성하여 맞춤 배포한다.
...
3. **업데이트 상향 호환 계약**: `applicationId` 일치, `versionCode` 단조 증가, 서명 호환성의 3 대 조건을 만족해야 기기 내 기존 데이터를 유지하며 업데이트가 이루어진다.
...
### 관련 세부 계약 문서
1. [Play app signing은 업로드 키와 앱 서명 키를 분리한다](play-app-signing.md)
2. [APK vs AAB 패키징 규격 및 배포 방식 비교](../apk-vs-aab.md)
3. [Play app signing은 업로드 키와 앱 서명 키를 분리한다](play-app-signing.md)
4. [App 업데이트는 application id, version code, 그리고 서명 호환성을 요구한다](app-update-compatibility.md)
5. [Google Play 테스트 트랙은 타깃 청중과 피드백 범위를 분리한다](google-play-testing-tracks.md)
6. [Internal app sharing은 배포 트랙이 아닌 빠른 아티팩트 공유다](internal-app-sharing.md)
7. [Staged rollout은 관측 가능한 배포 작업이다](staged-rollout-operations.md)
8. [In-app update의 flexible과 immediate 흐름은 블로킹에서 차이가 난다](in-app-update-flows.md)
9. [In-app review API는 리뷰를 요청할 뿐 보장하지 않는다](in-app-review-api.md)
10. [Play release 체크리스트는 아티팩트, 서명, 트랙, 롤백 조건을 동결한다](play-release-checklist.md)
