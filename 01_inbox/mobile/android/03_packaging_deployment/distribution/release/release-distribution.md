...
3. [App 업데이트는 application id, version code, 그리고 서명 호환성을 요구한다](app-update-compatibility.md)
4. [Google Play 테스트 트랙은 타깃 청중과 피드백 범위를 분리한다](google-play-testing-tracks.md)
5. [Internal app sharing은 배포 트랙이 아닌 빠른 아티팩트 공유다](internal-app-sharing.md)
6. [Staged rollout은 관측 가능한 배포 작업이다](staged-rollout-operations.md)
7. [In-app update의 flexible과 immediate 흐름은 블로킹에서 차이가 난다](in-app-update-flows.md)
8. [In-app review API는 리뷰를 요청할 뿐 보장하지 않는다](in-app-review-api.md)
9. [Play release 체크리스트는 아티팩트, 서명, 트랙, 롤백 조건을 동결한다](play-release-checklist.md)
