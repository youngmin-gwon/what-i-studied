...
안드로이드 권장 레이어드 아키텍처(Layered Architecture)의 핵심 원칙은 **UI Layer 는 선언적 렌더링(Rendering)만 담당하고, Domain Layer 는 비즈니스 정책(Policy/Rules)을 관장하며, Data Layer 는 데이터의 단일 출처([single source of truth](../../jetpack-compose/runtime/compose-ssot.md))를 보장**하여 관심사를 완벽히 분리하는 것이다.
...
1. **UI Layer (Rendering & State Interaction)**:
   Jetpack Compose UI 및 StateHolder([viewmodel](../state-management/viewmodel.md))로 구성된다. UI State 를 화면에 렌더링하고, 사용자의 인터랙션 이벤트를 인텐트/액션으로 변환하여 하위 레이어로 전달한다.
...
- 관련 계약 문서:
  ...
  - [ViewModel과 Repository는 UI Context를 보관하지 않는다](../context/viewmodel-repository-context-isolation.md)
