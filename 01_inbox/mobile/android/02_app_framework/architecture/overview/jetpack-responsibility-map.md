---
title: jetpack-responsibility-map
...
안드로이드 공식 **Jetpack Architecture 권장 사항은 특정 프레임워크(Room, Hilt, Navigation 등)를 강제로 사용해야 하는 고정된 필수 기술 스택(Mandatory Stack)이 아니다.** 본질은 **관심사 분리(Separation of Concerns), 단방향 데이터 흐름(Unidirectional Data Flow), 데이터 단일 출처([single source of truth](../../jetpack-compose/runtime/compose-ssot.md)) 원칙을 실현하기 위한 책임 분리 지도(Responsibility Map)**다.
...
- 관련 계약 문서:
  - [UI, domain, data layer는 rendering, policy, source of truth를 분리한다](layer-separation-principles.md)
