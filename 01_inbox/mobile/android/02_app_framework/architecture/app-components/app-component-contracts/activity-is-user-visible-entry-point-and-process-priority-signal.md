...
Activity는 단순한 화면 클래스가 아니다. 현재 visible/resumed Activity는 Android가 프로세스 중요도를 판단하는 강한 신호가 된다. 사용자가 보는 화면을 잃으면 앱 프로세스는 더 쉽게 회수될 수 있고, 이때 화면 상태와 영속 데이터 복구 전략이 필요해진다.
