...
Foreground Service는 사용자가 인지해야 하는 즉시성, 진행 중 작업을 OS에 알리는 계약이다. notification을 통해 사용자에게 노출되며, Android 버전과 target SDK에 따라 foreground-service type, permission, start restriction, timeout 조건이 달라진다.

따라서 "백그라운드에서 오래 실행하고 싶다"는 이유만으로 Foreground Service를 선택하면 안 된다. 음악 재생, active navigation, ongoing call, 사용자 시작 데이터 전송처럼 사용자 가시성과 즉시성이 있는지 먼저 확인해야 한다.
