...
Android 앱 아키텍처는 UI 패턴 이름보다 owner, lifetime, OS entry point를 먼저 나누는 문제다.

## 정본 지도

- [Android App Components](01_inbox/mobile/android/02_app_framework/architecture/app-components/android-app-components.md) - Activity, Service, BroadcastReceiver, ContentProvider를 OS entry point로 정리한다.
...
앱이 외부에서 어떻게 호출되는지가 궁금하면 app components로 간다. 어떤 Context를 전달해야 하는지가 궁금하면 context boundary로 간다. ViewModel, Flow, Room, WorkManager, Navigation, Hilt의 세부 구현은 Jetpack map에서 각 정본으로 이동한다.
