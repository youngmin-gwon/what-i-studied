...
상태를 변경할 수 있는 홀더는 ViewModel의 private 필드로 제한한다.
UI와 다른 호출자는 읽기 전용 `StateFlow`, `LiveData` 또는 동등한 인터페이스만 받는다.
...
Compose에서는 `StateFlow`와 `asStateFlow()` 조합을 기본으로 삼을 수 있다.

UI는 상태를 읽고 이벤트를 ViewModel 메서드로 전달한다.
상태 홀더를 public으로 열거나 mutable 컬렉션을 그대로 노출하지 않는다.

## API 점검

- `MutableStateFlow`와 `MutableLiveData`가 private인가?
...
상태 전이를 ViewModel의 명령과 검증 안에 모아,
UI와 비동기 작업이 서로 다른 값을 덮어쓰지 않게 하는 설계다.
