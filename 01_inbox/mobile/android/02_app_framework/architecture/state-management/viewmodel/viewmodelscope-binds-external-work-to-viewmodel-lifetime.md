---
title: viewmodelscope-binds-external-work-to-viewmodel-lifetime
tags: [android, android/architecture, android/state-management, android/viewmodel]
aliases: ["ViewModel은 외부 작업을 viewModelScope의 수명에 묶는다"]
date modified: 2026-08-03 17:28:06 +09:00
...
ViewModel 이 시작한 비동기 작업은 `viewModelScope` 에서 실행한다.

ViewModel 이 제거되면 스코프가 취소되어 작업도 함께 종료된다.
...
`GlobalScope` 나 ViewModel 과 무관한 독립 스코프를 사용하지 않는다.

그런 작업은 소유자가 불명확해지고 취소·테스트가 어려워진다.

작업이 ViewModel 보다 오래 살아야 한다면 Repository 나 애플리케이션 수준의
...
테스트에서는 외부 작업을 가짜 Repository 로 대체하고,

작업 완료 전 상태와 완료 후 상태를 확인한다.

ViewModel 이 정리될 때 작업이 취소되는지도 검증 대상이다.
