...
ViewModel은 기존 `ViewModelStore`에 남아 상태를 유지한다.
...
Fragment가 해당 소유자에서 제거되면 ViewModel은 정리 대상이 된다.
...
설정 변경만 견디면 되는 화면 상태는 ViewModel에 둔다.
프로세스 사망 뒤에도 복원해야 하는 작은 값은 `SavedStateHandle`에 둔다.
대량 데이터나 장기 데이터는 Repository와 영속 저장소가 담당한다.

ViewModel에 저장된 목록이 회전 후 남는다고 해서,
