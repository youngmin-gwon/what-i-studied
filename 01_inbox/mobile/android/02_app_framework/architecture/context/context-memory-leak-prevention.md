---
title: context-memory-leak-prevention
...
**`Context Leak` (메모리 누수)**의 근본 원인은 단순한 안드로이드 API 호출 실패가 아니라, **상대적으로 수명이 짧은 컴포넌트 Context(예: Activity Context) 참조가 수명이 더 긴 객체(예: Singleton, Static Variable, [viewmodel](../state-management/viewmodel.md), Background Coroutine Scope)에 의해 캡처되어 지속적으로 참조되는 수명 불일치(Lifetime Mismatch)**에 있다.
