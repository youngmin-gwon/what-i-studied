...
`Pager` 는 `PagingConfig` 와 `PagingSource` factory 를 받아 `Flow<PagingData<T>>` 를 노출한다. 이 흐름은 UI 가 직접 page key 를 관리하지 않고, 필요한 시점에 page loading 을 요청하게 만드는 [viewmodel](../../architecture/state-management/viewmodel.md)-facing API 다.
