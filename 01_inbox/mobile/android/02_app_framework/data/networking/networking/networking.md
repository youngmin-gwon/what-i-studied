...
- [Retrofit 인터페이스는 API 계약을 선언하고 OkHttp가 실제 전송을 담당한다](./retrofit-interface-declares-api-while-okhttp-executes-transport.md)
...
- [gRPC는 Protobuf 기반 강타입 스트리밍 계약을 선언하고 REST는 단발성 request-response 계약에 머문다](./grpc-declares-typed-streaming-while-rest-stays-single-shot-request-response.md)
...
- 로컬 우선 쓰기, WorkManager 지연 동기화, 충돌 해결 정책은 [Learning Spine 8장](../../../../00_foundations/learning-spine/08-data-storage-network-and-offline-recovery.md)과 [영속 저장소 계약](../../storage/persistence/persistence.md) 에 둔다. 이 클러스터는 그 흐름 중 "네트워크 클라이언트 계층"만 담당한다.
