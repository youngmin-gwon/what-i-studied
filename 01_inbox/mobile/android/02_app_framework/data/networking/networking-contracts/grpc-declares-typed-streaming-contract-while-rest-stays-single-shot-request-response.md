...
관련 노트: [Retrofit 인터페이스는 API 계약을 선언하고 OkHttp가 실제 전송을 담당한다](./retrofit-interface-declares-api-contract-while-okhttp-executes-transport.md), [suspend API 호출의 취소는 호출자의 coroutine scope를 따라간다](./suspend-api-call-cancellation-follows-the-callers-coroutine-scope.md)
