---
title: network-call-cancellation
...
Retrofit interface 메서드를 `suspend fun` 으로 선언하면, 그 호출을 감싸고 있는 coroutine 이 취소될 때 진행 중인 네트워크 요청도 실제로 중단된다. 이것은 우연이 아니라 Retrofit 이 내부적으로 `suspendCancellableCoroutine` 으로 OkHttp `Call` 을 감싸고, coroutine 취소를 `Call.cancel()` 호출로 연결해 두었기 때문이다. [Structured concurrency는 부모 scope가 자식 작업의 수명을 소유하게 한다](../../../../../computer-science/structured-concurrency.md)는 계약이 네트워크 호출까지 그대로 이어지는 지점이다.
