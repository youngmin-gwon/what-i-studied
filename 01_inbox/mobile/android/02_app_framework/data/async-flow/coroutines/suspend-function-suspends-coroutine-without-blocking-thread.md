...
`suspend` 는 thread 전환을 자동으로 의미하지 않는다. 무거운 작업은 `withContext(Dispatchers.IO)` 나 적절한 dispatcher 선택과 함께 설계해야 한다. 실행 위치는 [Dispatcher는 실행 위치를 고르고 Scope는 작업 수명을 소유한다](./dispatcher-selects-execution-context-not-work-lifetime.md) 의 관심사다.
