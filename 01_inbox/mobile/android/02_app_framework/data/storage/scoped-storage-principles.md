---
title: scoped-storage-principles
...
Android 10 부터 앱은 자신의 파일을 직접 다루고, 공유 파일은 플랫폼 API 를 통한다.
...
3. 일반 문서와 사용자 지정 폴더는 SAF picker 로 사용자의 선택을 받는다.
...
사진 한두 장을 고르는 기능은 [Photo Picker](photo-picker-media-access.md) 를 먼저 검토한다.
...
MediaStore 와 SAF 는 URI 와 ContentResolver 를 통해 제공자 경계를 존중한다.

URI 를 받았다고 해서 그 자원이 영구적으로 존재한다고 가정하지 않는다.

스트림이 열리지 않거나 권한이 철회된 경우를 사용자 흐름의 정상 오류로 처리한다.

Scoped Storage 는 단순한 제약이 아니라 데이터 공개 범위를 코드에 반영하는 계약이다.
