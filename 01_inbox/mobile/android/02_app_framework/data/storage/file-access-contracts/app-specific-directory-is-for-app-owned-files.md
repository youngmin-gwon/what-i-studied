...
이미지나 영상이라는 형식만으로 MediaStore를 선택하지 않는다.
...
저장소 객체가 오래 살아야 하면 Activity가 아닌 Application Context를 전달한다.
파일 경로를 문자열로 조립하지 말고 Context가 제공하는 디렉터리를 사용한다.
