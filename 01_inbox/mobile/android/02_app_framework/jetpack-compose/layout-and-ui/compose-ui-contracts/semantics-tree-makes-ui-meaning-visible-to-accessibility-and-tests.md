...
Semantics Tree 는 화면의 픽셀 구조가 아니라 UI 의 의미를 접근성 서비스와 Compose 테스트가 읽을 수 있는 형태로 제공한다. Text, Button 같은 기본 컴포넌트는 많은 semantics 를 자동으로 제공하지만, custom component 는 역할, 상태, 설명, action 을 명시해야 할 수 있다.
