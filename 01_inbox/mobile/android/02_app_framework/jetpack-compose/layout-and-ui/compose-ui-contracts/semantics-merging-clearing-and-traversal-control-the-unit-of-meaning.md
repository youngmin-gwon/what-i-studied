...
`clearAndSet**Semantics**(접근성 서비스 및 UI 테스트 프레임워크가 읽을 수 있도록 UI 요소의 의미적 정보를 캡슐화한 정보 트리)` 는 기존 semantics 를 지우고 새 의미로 대체할 때 쓰며, 장식 요소를 접근성 서비스에서만 숨기는 `hideFromAccessibility` 와 다르다. 두 API 는 목적이 다르므로 "숨기기"로 뭉개면 안 된다.
