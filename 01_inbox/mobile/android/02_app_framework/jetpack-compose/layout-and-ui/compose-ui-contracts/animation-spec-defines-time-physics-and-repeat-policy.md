...
관련 노트: [Compose animation API is selected by change unit and control level](./compose-animation-api-is-selected-by-change-unit-and-control-level.md), [Value animation APIs separate single target transition infinite and coroutine control](./value-animation-apis-separate-single-target-transition-infinite-and-coroutine-control.md)
