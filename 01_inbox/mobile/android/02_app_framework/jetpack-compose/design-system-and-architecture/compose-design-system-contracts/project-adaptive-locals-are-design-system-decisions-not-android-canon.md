...
`LocalWindowSizeClass`, `LocalAdaptiveInfo` 같은 Jetpack 제공 Local 외에, 프로젝트가 자체적으로 정의한 `CompositionLocal` 값(예: 화면 크기 구간, 레이아웃 모드, 브랜드 토큰)은 Android SDK 나 Compose 표준 API 가 아니라 프로젝트의 design-system 결정이다.
