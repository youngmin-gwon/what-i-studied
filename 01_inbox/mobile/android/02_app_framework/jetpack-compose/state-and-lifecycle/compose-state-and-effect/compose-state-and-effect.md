...
Composable 함수는 순수하고 예측 가능하며 [부수 효과](../compose-side-effect.md)가 없어야 한다([Composable Body Purity](../../runtime/compose-runtime/composable-body-purity.md)). 화면 진입, 이탈, 수명주기 변경 또는 외부 비동기 스트림 수신 시 발생하는 [부수 효과](../compose-side-effect.md)는 반드시 전용 Effect API (`LaunchedEffect`, `DisposableEffect`, `rememberCoroutineScope` 등) 내부에 캡슐화되어 렌더링 파이프라인의 오염을 막아야 한다.
...
- [Composable Body Purity](../../runtime/compose-runtime/composable-body-purity.md) - Pure Composable 함수 준칙
