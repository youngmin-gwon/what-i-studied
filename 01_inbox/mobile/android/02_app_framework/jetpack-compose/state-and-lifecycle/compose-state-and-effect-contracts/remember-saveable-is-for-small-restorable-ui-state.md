...
date modified: 2026-08-05 18:55:01 +09:00
...
`rememberSaveable` 은 일반 `remember` 와 달리 **화면 회전(Activity Recreation) 및 안드로이드 OS 에 의한 프로세스 강제 종료(System-initiated Process Death) 시에도 상태 값을 Android `Bundle` 인스턴스에 저장하여 복원하는 API**다.

---

### 2. rememberSaveable 과 경량 상태 규약의 필요성 (Why)
