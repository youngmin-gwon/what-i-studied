...
`LaunchedEffect`는 composition에 들어올 때 coroutine을 시작하고, key가 바뀌거나 Composable이 사라지면 작업을 취소한다.
...
- 작업이 특정 key에 종속된다.
- key가 바뀌면 이전 작업을 취소하고 새 작업을 시작해야 한다.
- Composable이 제거되면 작업도 더 이상 의미가 없다.
...
`itemId`가 바뀌면 기존 effect가 취소되고 새 effect가 시작된다.
화면 진입 시 한 번만 실행하는 작업은 고정 key를 사용할 수 있지만, 실제로 고정 수명이 맞는지 확인한다.
...
recomposition마다 네트워크 요청이나 저장이 반복될 수 있기 때문이다.

## ViewModel과의 경계

화면 데이터의 source of truth와 장기 비즈니스 작업은 ViewModel 또는 repository가 소유한다.
...
`LaunchedEffect`는 effect의 owner를 Composable로 만든다.
`viewModelScope`는 작업의 owner를 ViewModel로 만든다.
