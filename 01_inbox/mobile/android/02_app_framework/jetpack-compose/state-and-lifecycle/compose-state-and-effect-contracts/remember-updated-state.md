...
tags: [android, compose, rememberUpdatedState, side-effects, long-running-effects, callbacks]
...
1. **타이머 / 애니메이션 수명주기 내 콜백 바인딩**:
   - `LaunchedEffect(Unit)` 으로 3초 타이머를 켜두었을 때, 3초 뒤 실행될 `onTimeout` 람다가 Recomposition 으로 교체되어도 타이머를 취소하지 않고 최신 `onTimeout` 을 호출하고자 할 때.
2. **`key` 재시작 오버헤드 방지**:
