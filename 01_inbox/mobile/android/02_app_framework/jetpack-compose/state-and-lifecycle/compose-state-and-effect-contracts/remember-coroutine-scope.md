...
tags: [android, compose, rememberCoroutineScope, side-effects, coroutines, events, click-handlers]
...
Composable 함수 몸체 내부가 아닌 **"콜백 이벤트 핸들러 내부"**에서 서스펜드(Suspend) 함수를 호출해야 할 때 사용한다. 이 스코프에서 생성된 코루틴은 해당 Composable 이 화면에서 이탈(Disposition)하면 자동으로 일괄 cancellation 되어 메로리 누수나 고아 작업(Orphan Job)을 예방한다.
...
- [Kotlin Coroutines](../../../kotlin-coroutines.md) - 코루틴 동시성 엔진
