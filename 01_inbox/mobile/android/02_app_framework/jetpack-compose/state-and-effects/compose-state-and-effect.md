---
title: compose-state-and-effect
...
**Compose State & Effect Contracts** 는 Jetpack Compose 의 단방향 데이터 흐름([Compose SSOT](../runtime/compose-ssot.md)) 및 컴포지션 수명주기(Composition Lifecycle) 내에서 **UI 상태(State)를 안전하게 유지·복원하고, 스레드 [부수 효과](compose-side-effect.md)(Side-Effects)를 제어하기 위한 표준 규약 묶음 지도**이다.

Composable 함수는 순수하고 예측 가능하며 [부수 효과](compose-side-effect.md)가 없어야 한다([Composable Body Purity](../runtime/composable-body-purity.md)). 화면 진입, 이탈, 수명주기 변경 또는 외부 비동기 스트림 수신 시 발생하는 [부수 효과](compose-side-effect.md)는 반드시 전용 Effect API (`LaunchedEffect`, `DisposableEffect`, `rememberCoroutineScope` 등) 내부에 캡슐화되어 렌더링 파이프라인의 오염을 막아야 한다.
