...
잘못된 State API 선택은 화면 회전(Configuration Change) 시 상태가 파괴되거나, 프로세스 수거(System-initiated Process Death) 시 유저 데이터가 날아가는 버그를 발생시킨다. 반대로 모든 소형 UI 상태를 ViewModel 에 몰아넣으면 [Compose SSOT](../runtime/compose-ssot.md) 계층 구조가 비대해진다.
...
| **복원 방식** | 복원 불가 (메모리 갱신) | `Bundle` (SavedStateRegistry) 직렬화 복원 | [Compose SSOT](../runtime/compose-ssot.md) 및 `SavedStateHandle` |
...
- [Compose SSOT](../runtime/compose-ssot.md) - 단방향 데이터 흐름 아키텍처
- [ViewModel](../../architecture/state-management/viewmodel.md) - 안드로이드 표준 비즈니스 상태 홀더
