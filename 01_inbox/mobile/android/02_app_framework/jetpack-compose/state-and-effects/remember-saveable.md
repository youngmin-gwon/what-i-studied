...
3. **적합하지 않은 무거운 데이터**:
   - `Bundle` 제한 용량(약 1MB 이내) 때문에 대용량 리스트나 비트맵 이미지는 `rememberSaveable` 에 넣으면 안 되며, [ViewModel](../../architecture/state-management/viewmodel.md) 및 `SavedStateHandle` 로 관리해야 한다.
