...
`remember` 는 일반 memoization cache 가 아니라 Composition 의 특정 호출 위치에 귀속된 저장공간이다. **Recomposition**(상태 변경 시 영향을 받는 Composable 스코프만 선택적으로 재실행하여 UI 트리를 갱신하는 과정) 사이에는 값을 유지하지만, 해당 call site 가 Composition 에서 제거되면 값도 잊힌다.
...
`remember` 는 설정 변경, 시스템 주도 process recreation, 앱 재시작까지 보존하는 장치가 아니다. 작은 UI 복원 상태는 `**rememberSaveable**(화면 회전이나 프로세스 재시작 후에도 Bundle을 통해 UI 상태를 복원해 주는 저장 API)`, 화면 상태는 ViewModel/state holder, 영구 데이터는 persistence layer 가 후보가 된다.
