---
title: remember is Composition-scoped storage, not a general cache
...
# remember is Composition-scoped storage, not a general cache

`remember`는 일반 memoization cache가 아니라 Composition의 특정 호출 위치에 귀속된 저장공간이다. Recomposition 사이에는 값을 유지하지만, 해당 call site가 Composition에서 제거되면 값도 잊힌다.

Key를 넘기면 key 변화가 저장값의 identity를 바꾼다. 이때 `remember` block은 다시 실행되고 이전 값은 더 이상 같은 저장공간으로 취급되지 않는다.

`remember`는 설정 변경, 시스템 주도 process recreation, 앱 재시작까지 보존하는 장치가 아니다. 작은 UI 복원 상태는 `rememberSaveable`, 화면 상태는 ViewModel/state holder, 영구 데이터는 persistence layer가 후보가 된다.
