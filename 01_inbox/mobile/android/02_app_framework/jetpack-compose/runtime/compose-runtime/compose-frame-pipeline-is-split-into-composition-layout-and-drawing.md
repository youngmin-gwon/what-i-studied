...
관련 노트: [Snapshot State 관찰은 State를 읽은 scope를 invalidation 대상으로 만든다](./snapshot-state-observation-invalidates-state-read-scopes.md), [Compose 상태 읽기 위치는 recomposition 범위 제어로 직결된다](../../performance/compose-performance/compose-state-read-location-controls-recomposition-scope.md)
