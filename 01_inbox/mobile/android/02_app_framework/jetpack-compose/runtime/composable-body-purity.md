...
- **연산 메모제이션**: 계산 비용이 큰 로직은 `remember { … }`나 `derivedStateOf { … }`로 메모제이션하거나 [ViewModel](../../architecture/state-management/viewmodel.md) 계층으로 이관한다.
...
- [LaunchedEffect 실행 규약](../state-and-effects/launched-effect.md) - 취소 가능한 부작용을 관리하는 LaunchedEffect
- [Composition 내 무거운 작업 처리 규약](../performance/composition-heavy-work-offloading.md) - Fast 규약 준수를 위한 성능 가이드
- [ViewModel](../../architecture/state-management/viewmodel.md) - 무거운 데이터 작업과 비즈니스 로직을 이관받는 상태 관리자
