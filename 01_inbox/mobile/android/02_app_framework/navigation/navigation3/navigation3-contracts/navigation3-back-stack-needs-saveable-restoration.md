...
- key에는 화면 복원에 필요한 최소 식별자만 둔다.
- detail 화면의 form state, scroll state, loading state는 별도 UI state로 분리한다.
- deep link는 최종 key만 push하지 말고 필요한 root stack을 함께 구성한다.
- process death 복원 뒤 사용할 수 없는 key는 fallback destination으로 수렴시킨다.
