---
title: navigation3-transitions-back-policy
...
1. **개념 (What)**:
   - Navigation 3 화면 전환 모션 애니메이션(Transition)과 뒤로 가기 및 Predictive Back 제스처 정책(Back Policy)은 반드시 **동일한 `NavBackStack` 진실의 단일 원천([single source of truth](../../jetpack-compose/runtime/compose-ssot.md)) 상태**를 공유하여 작동해야 한다는 원칙이다.
2. **필요성 (Why)**:
...
- 연관 계약: [NavKey와 back stack은 앱이 소유하는 navigation 상태다](navkey-back-stack-ownership.md)
