# AndroidManifest.xml은 OS에 앱의 컴포넌트를 선언한다
...
AndroidManifest.xml은 개발자와 Android OS 사이의 설치·실행 계약이다.
앱 코드가 존재하는 것만으로 OS가 모든 컴포넌트를 실행할 수 있는 것은 아니다.
OS는 매니페스트를 읽고 앱의 컴포넌트, 권한, 진입점, 메타데이터를 파악한다.
...
| `<provider>` | ContentProvider와 데이터 경계 |
...
권한과 exported 설정은 다른 UID의 호출이 가능한지 판단하는 입력이 된다.
...
매니페스트는 설정 파일을 넘어 앱이 OS에 노출하는 컴포넌트 경계다.
