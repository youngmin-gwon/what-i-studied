...
Intent는 안드로이드 컴포넌트 사이에서 작업을 요청하는 메시지 객체다.
호출자는 실행할 작업과 입력을 표현하고, 시스템은 적절한 컴포넌트에 전달한다.
따라서 Intent는 단순한 화면 이동 명령보다 넓은 IPC 계약으로 이해해야 한다.
...
| `flags` | 실행 방식 제어 | task와 back stack 제어 |
...
URI는 위치나 식별자를 표현하고 MIME 타입은 데이터 형식을 표현한다.
...
4. 민감한 값을 암시적 Intent로 노출하고 있지 않은가?
5. 링크나 URI를 받은 뒤 허용된 scheme과 host를 확인하는가?

## 정리

Intent는 실행 대상 자체가 아니라 실행 의도를 표현하는 데이터 구조다.
시스템 라우팅이 필요한 경우에는 필드와 Manifest의 필터가 함께 계약을 이룬다.
