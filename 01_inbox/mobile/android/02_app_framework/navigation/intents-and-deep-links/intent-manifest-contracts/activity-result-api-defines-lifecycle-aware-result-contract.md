...
- result callback은 UI controller 수명과 복원 상태를 함께 고려한다.
- 외부 Activity가 반환하는 data를 내부 trusted state처럼 바로 취급하지 않는다.
- Photo Picker, SAF, permission request는 각각 storage/permission 정본과 연결한다.
