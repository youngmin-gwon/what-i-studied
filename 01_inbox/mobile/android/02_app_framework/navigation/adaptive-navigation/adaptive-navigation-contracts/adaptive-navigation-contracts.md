...
Adaptive Navigation은 화면 크기와 입력 환경에 따라 app chrome과 content 배치를 바꾸는 문제다. Navigation 3의 back stack 상태와 adaptive scaffold의 표시 정책을 분리해서 읽는다.
...
- route/back stack은 앱 navigation state가 소유하고, chrome과 pane 배치는 window state가 결정한다.
- compact/expanded 전환은 같은 목적지를 다른 배치로 표현해야 하며 다른 route 체계로 갈라지면 안 된다.
- 표준 Material adaptive scaffold가 해결하는 문제인지 먼저 확인한 뒤 custom layout을 선택한다.
