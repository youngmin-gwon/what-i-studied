...
Compose 목록에서 중요한 것은 stable key 와 state 위치다. item 내부 state 가 목록 이동, 삽입, 삭제 중에도 보존되어야 하면 key 를 명시하고, 화면 전체 상태는 [viewmodel](../../architecture/state-management/viewmodel.md) 이나 상위 state holder 가 소유한다.
