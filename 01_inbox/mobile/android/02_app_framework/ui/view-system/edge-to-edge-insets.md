...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge() // 시스템 바 뒤까지 그리기 시작
        setContent {
            Scaffold(modifier = Modifier.safeDrawingPadding()) { /* content */ }
        }
    }
}
```

`enableEdgeToEdge()` 를 빼면 target SDK 35 기기에서는 여전히 edge-to-edge 로 그려지지만 `safeDrawingPadding()` 같은 insets 처리 없이는 버튼이나 텍스트가 status bar/navigation bar 뒤에 가려지는 것을 실기기나 에뮬레이터에서 바로 확인할 수 있다.
...
관련 노트: [Navigation 3 transition과 predictive back은 같은 stack state를 기준으로 해야 한다](../../navigation/navigation3/navigation3-transitions-back-policy.md), [Window와 posture가 Adaptive Navigation의 입력이다](../../navigation/adaptive/adaptive-window-posture.md)
