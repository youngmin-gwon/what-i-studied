...
- 앱이 완전히 통제하는 자체 도메인의 정적 콘텐츠만 로드한다면 Safe Browsing opt-out 을 검토할 수 있지만, 이 경우에도 `addJavascriptInterface()` 브리지를 열었다면 [addJavascriptInterface()는 @JavascriptInterface로 표시한 메서드만 웹 콘텐츠에 노출한다](./addjavascriptinterface-exposes-only-annotated-methods-to-web-content.md) 의 origin 제한이 여전히 필요하다.
