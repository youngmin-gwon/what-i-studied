---
title: locale-resource-qualifiers
...
시스템(또는 [Android 13+ 앱별 언어 설정으로 setApplicationLocales가 시스템 로케일과 별개로 앱 언어를 바꾼다](per-app-language-preferences.md) 로 지정된) 로케일을 기준으로, 리소스 프레임워크는 가장 구체적으로 일치하는 qualifier 디렉터리를 찾는다. 정확히 일치하는 언어+지역 조합이 없으면 언어만 일치하는 디렉터리로, 그마저 없으면 qualifier 없는 기본 `values/` 로 폴백한다.
...
관련 노트: [Android 13+ 앱별 언어 설정으로 setApplicationLocales가 시스템 로케일과 별개로 앱 언어를 바꾼다](per-app-language-preferences.md)
