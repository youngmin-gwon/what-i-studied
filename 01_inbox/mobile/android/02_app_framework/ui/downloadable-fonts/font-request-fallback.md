...
- 이 노트는 요청 API 형태와 실패 처리 계약만 다룬다. 폰트를 요청 시점에 가져오는 배달 모델 자체(왜 APK에 번들하지 않는가)는 [Downloadable Fonts는 폰트를 APK에 번들하지 않고 폰트 제공자 앱에 요청 시점에 위임한다](font-provider-delivery.md)가 다룬다.
