...
1. [Downloadable Fonts는 폰트를 APK에 번들하지 않고 폰트 제공자 앱에 요청 시점에 위임한다](font-provider-delivery.md)에서 APK 크기 절감과 제공자 캐시 공유라는 이 API의 존재 이유를 먼저 본다.
2. [폰트 요청은 XML font-family 선언이나 FontRequest 코드 경로를 따르며 실패 시 폴백이 필요하다](font-request-fallback.md)에서 두 요청 경로와 `onTypefaceRequestFailed()` 실패 처리 계약을 본다.
...
- [Downloadable Fonts는 폰트를 APK에 번들하지 않고 폰트 제공자 앱에 요청 시점에 위임한다](font-provider-delivery.md)
- [폰트 요청은 XML font-family 선언이나 FontRequest 코드 경로를 따르며 실패 시 폴백이 필요하다](font-request-fallback.md)
