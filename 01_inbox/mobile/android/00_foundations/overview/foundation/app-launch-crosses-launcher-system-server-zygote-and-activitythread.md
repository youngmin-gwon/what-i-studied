...
안드로이드의 앱 실행(특히 프로세스가 완전히 새로 뜨는 **Cold Launch**)은 Launcher 앱부터 [시스템 서비스 (`system_server`)](../../../01_system_internals/boot-and-runtime/system-server/system-server.md), [마스터 프로세스 (`Zygote`)](../../../01_system_internals/boot-and-runtime/zygote-runtime/zygote-runtime.md), 그리고 앱의 메인 스레드인 [`ActivityThread`](../../../02_app_framework/architecture/app-components/activity-thread.md) 까지 시스템의 여러 계층을 교차하며 일어나는 정교한 프로세스 생명주기 여정이다.

---

## 1. 앱 실행 4 단계 주요 흐름 (Cold Launch Pipeline)

1. **Launcher ➔ [`system_server`](../../../01_system_internals/boot-and-runtime/system-server/system-server.md) (시작 요청)**:
   ...
   - `system_server`는 해당 앱의 [보안 권한](../../../05_security_privacy/permissions/appops-and-permissions.md)과 [앱 프로세스 존재 여부](../../../01_system_internals/boot-and-runtime/zygote-runtime/zygote-runtime.md) 를 검사한다.
2. **[`system_server`](../../../01_system_internals/boot-and-runtime/system-server/system-server.md) ➔ [`Zygote`](../../../01_system_internals/boot-and-runtime/zygote-runtime/zygote-runtime.md) (프로세스 생성 요청)**:
...
4. **`Application` 및 `Activity` 라이프사이클 실행 ➔ 화면 표시 ([TTID & TTFD](../../../06_testing_performance/performance/ttid-and-ttfd.md))**:
   - `Application.onCreate()`와 `Activity.onCreate() ~ onResume()` 이 순차적으로 실행된다.
   - 첫 번째 프레임이 화면 렌더링 시스템(`WindowManager` / `SurfaceFlinger`)에 전달되어 첫 화면이 노출되는 시점을 [TTID (Time To Initial Display)](../../../06_testing_performance/performance/ttid-and-ttfd.md) 라 부르며, 실제 사용 가능함을 알리는 시점을 [TTFD (Time To Fully Drawn)](../../../06_testing_performance/performance/ttid-and-ttfd.md) 라 부른다.
...
| :--- | :--- | :--- |
| **Intent 및 권한 검사** | `ActivityNotFoundException`, `SecurityException` | `AndroidManifest.xml` 내 `exported` 설정 및 [AppOps / 권한](../../../05_security_privacy/permissions/appops-and-permissions.md) 확인 |
...
| **Activity Lifecycle** | `onCreate()` 진입 후 화면 멈춤 (ANR) | [ActivityThread 메인 스레드](../../../02_app_framework/architecture/app-components/activity-thread.md) 블로킹, 교착 상태([Deadlock](../../../../../computer-science/deadlock.md)) 또는 DB 락 |
...
- [TTID & TTFD 성능 지표](../../../06_testing_performance/performance/ttid-and-ttfd.md) - 앱 구동 2 대 성능 측정 지표
...
- [AppOps & 권한 레퍼런스](../../../05_security_privacy/permissions/appops-and-permissions.md) - 앱 실행 시 권한 검사 통제
