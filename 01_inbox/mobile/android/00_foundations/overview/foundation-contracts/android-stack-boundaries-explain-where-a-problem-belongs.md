...
동일한 `startActivity()`나 카메라 `takePicture()` API라 하더라도 호출이 막힌 지점에 따라 [앱 프레임워크](../../../02_app_framework/viewmodel.md) 문제인지, [시스템 서비스(`system_server`)](../../../04_system_services/system-server.md) 문제인지, [하드웨어 추상화 계층(`HAL`)](../../../01_system_internals/hal.md) 문제인지, [리눅스 커널](../../../../../../operating-systems/linux-kernel.md) 디바이스 드라이버 문제인지 소유 책임 영역이 완전히 달라진다.
...
1. **App Layer Boundary (앱 계층 경계)**:
   - 앱 내 코드, UI State, [메인 스레드](../../../../../../02_references/computer-science/thread.md) 블로킹(ANR), NPE/NullPointerException 등.
2. **Framework & IPC Boundary (프레임워크 및 IPC 경계)**:
   - [Binder IPC](../../../01_system_internals/binder-ipc.md)를 거치는 과정에서의 [권한 및 AppOps](../../../05_security_privacy/appops-and-permissions.md) 거부, `SecurityException`, `ActivityNotFoundException`.
3. **System Service Boundary (시스템 서비스 경계)**:
   - [`system_server`](../../../04_system_services/system-server.md) 내의 `ActivityManagerService(AMS)`, `WindowManagerService(WMS)` 등에서 발생한 큐 타임아웃, 교착 상태([Deadlock](../../../../../../02_references/computer-science/thread.md)).
4. **Hardware & Kernel Boundary (하드웨어 및 커널 경계)**:
   - [`HAL`](../../../01_system_internals/hal.md) 하드웨어 세션 에러, [`Linux Kernel`](../../../../../../operating-systems/linux-kernel.md) dmesg 에러, GPU 랜더링 펜스(Fence) 타임아웃.
...
| Component 콜백 진입 | [메인 스레드](../../../../../../02_references/computer-science/thread.md) 멈춤 (ANR) 또는 UI State 불일치 | 앱 프레임워크 / [ViewModel](../../../02_app_framework/viewmodel.md) |
| [Binder IPC](../../../01_system_internals/binder-ipc.md) 요청 전송 | `TransactionTooLargeException` 또는 [system_server](../../../04_system_services/system-server.md) 타임아웃 | IPC / [system_server](../../../04_system_services/system-server.md) |
| Native Service 세션 생성 | `CameraAccessException` 또는 [HAL](../../../01_system_internals/hal.md) status 에러 | [HAL](../../../01_system_internals/hal.md) / Media Native Runtime |
...
1. `FirebaseMessagingService.onMessageReceived()`가 호출되었는가?
   ...
   - **호출 안 됨**: 앱 프로세스가 죽었거나 Doze 모드 제약 ➔ FCM 서버 응답, 토큰 상태, 백그라운드 [Zygote](../../../01_system_internals/zygote.md) 프로세스 상태 경계 조사.
...
- [HAL 레퍼런스](../../../01_system_internals/hal.md) - 제조사 하드웨어 추상화 경계
- [system_server 레퍼런스](../../../04_system_services/system-server.md) - 안드로이드 핵심 시스템 서비스 경계
- [Binder IPC 레퍼런스](../../../01_system_internals/binder-ipc.md) - 프로세스 및 계층 간 통신 경계
