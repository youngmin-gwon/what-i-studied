...
- [Looper와 Handler는 스레드의 메시지 큐를 관리하고 이벤트를 순차적으로 처리한다](../../02_app_framework/async-flow/handler-looper-message-queue.md)
...
- [Coroutine은 thread가 아니라 취소 가능한 경량 작업이다](../../02_app_framework/async-flow/coroutines/coroutine-lightweight-concurrency.md)
- [Dispatcher는 실행 위치를 고르고 Scope는 작업 수명을 소유한다](../../02_app_framework/async-flow/coroutines/coroutine-dispatchers.md)
- [Structured concurrency는 부모 scope가 자식 작업의 수명을 소유하게 한다](../../02_app_framework/async-flow/coroutines/structured-concurrency.md)
