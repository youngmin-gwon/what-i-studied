...
| :--- | :--- | :--- | :--- |
| **Input Dispatching Time** | 이벤트 수신 후 UI 반응 < 100ms | 이벤트 미처리 상태 > 5000ms 지속 | 메인 스레드 내 복잡한 계산/동기 DB 접근을 [Coroutines](../../02_app_framework/async-flow/coroutines/kotlin-coroutines.md) `Dispatchers.Default` / `IO` 로 이관 |
