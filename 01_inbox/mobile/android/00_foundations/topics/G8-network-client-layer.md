...
이 문서는 Android 앱이 백엔드 서버와 통신하기 위해 구축하는 네트워크 계층의 설계 및 구현 방식을 종합합니다. Retrofit의 선언적 API, OkHttp의 전송 계층 제어, 그리고 Coroutine과 연동된 취소 정책을 다룹니다.
...
앱의 네트워크 계층은 서버와의 통신 규약(Contract)을 명시적으로 선언하고, 이를 실제 전송 계층에서 신뢰성 있게 처리하는 구조를 가집니다. 공통 관심사(인증, 로깅)는 파이프라인 중간에 주입되며, 호출의 생명주기는 UI 상태와 동기화됩니다.
