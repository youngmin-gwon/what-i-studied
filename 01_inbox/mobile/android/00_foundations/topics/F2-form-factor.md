...
- [Wear OS 앱은 컴패니언 휴대전화 앱과 독립적으로 실행될 수 있다](../../07_platforms/wear/standalone-wear-os-apps.md)

#### 3.2. Android TV (10-Foot UI)
TV 앱은 터치스크린이 없음을 명시적으로 선언해야 하며, 사용자가 10피트 떨어져서 리모컨(D-Pad)으로 조작하므로 요소 간의 명확한 포커스 기반 내비게이션이 필수입니다.
- [10-Foot UI는 포커스 기반 내비게이션을 요구한다](../../07_platforms/tv/10-foot-ui-focus-navigation.md)
...
- [Android Auto는 프로젝션이고, Android Automotive OS는 임베디드 OS이다](../../07_platforms/auto/android-auto-vs-automotive-os.md)
...
- [ChromeOS는 데스크톱 윈도우에 매핑된 컨테이너/가상머신에서 안드로이드 앱을 실행한다](../../07_platforms/chromeos/chromeos-container-window-mapping.md)

#### 3.5. Android XR (공간 컴퓨팅)
XR(혼합현실) 환경은 기존 2D 평면을 넘어선 공간적 폼 팩터입니다. 앱은 런타임에 기기의 공간 표시(Spatial) 능력을 확인하고 3D 상호작용에 대응해야 합니다.
- [Android XR은 단순 평면 포트가 아닌 공간적 폼 팩터이다](../../07_platforms/xr/android-xr-spatial-computing.md)
