...
- [테스트 계층은 피드백 비용과 리스크에 의해 선택된다](../../06_testing_performance/testing/test-pyramid-strategy.md)

#### 3.2. 테스트 계층별 실패 신호
단위 테스트는 로직의 결함을 짚어내고, 통합 테스트는 모듈 간의 연결 실패를, UI/E2E 테스트는 시스템 전체의 사용자 경험 단절을 알려주는 서로 다른 실패 신호를 제공합니다.
- [단위, 통합, UI, E2E 테스트는 서로 다른 실패 신호를 갖는다](../../06_testing_performance/testing/test-levels-failure-signals.md)
...
- [테스트 더블은 행동 소유권에 따라 Fake와 Mock을 선택한다](../../06_testing_performance/testing/test-doubles-fake-vs-mock.md)
...
- [Espresso는 비동기 대기를 위한 Idling Resource와 함께 동기적 뷰 UI 테스트를 담당한다](../../06_testing_performance/testing/espresso-idling-resources.md)
...
- [Compose UI 테스트는 안정적인 선택자와 시맨틱을 사용해야 한다](../../06_testing_performance/testing/compose-ui-tests-semantics.md)
