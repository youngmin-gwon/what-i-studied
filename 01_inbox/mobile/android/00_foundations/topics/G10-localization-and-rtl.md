...
- [리소스 한정자(Resource Qualifiers)는 런타임 로케일에 따라 문자열을 선택함](../../02_app_framework/ui/localization/locale-resource-qualifiers.md): `-en`, `-ko` 등의 폴더 구조를 통해 기기의 언어 설정에 따라 올바른 텍스트와 자산을 제공하는 기본 메커니즘을 정의합니다.
- [앱별 언어 설정(setApplicationLocales)은 시스템 로케일을 무시함](../../02_app_framework/ui/localization/per-app-language-preferences.md): 기기의 기본 언어와 독립적으로 특정 앱에만 다른 언어를 적용할 수 있도록 지원하는 Android 13 이상(및 하위 호환)의 설정을 다룹니다.
- [RTL 미러링은 start/end에 대해 자동이지만 아이콘은 수동 처리됨](../../02_app_framework/ui/localization/rtl-layout-mirroring.md): 좌우 여백과 배치는 `start`/`end` 속성으로 자동 반전되지만, 벡터 드로어블의 방향성은 명시적으로 반전 속성을 선언해야 함을 설명합니다.
