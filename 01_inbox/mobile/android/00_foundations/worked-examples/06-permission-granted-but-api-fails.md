...
2. **App Framework & IPC Layer (Background Transition & Permission Gate Split)**
   - 사용자가 앱을 떠나 백그라운드로 전환되는 순간, `FusedLocationProviderClient` 는 위치 업데이트 요청을 [binder ipc](../../01_system_internals/ipc-and-process/binder-ipc.md) 를 통해 System Server 의 `LocationManagerService` (LMS) 로 전달한다.
