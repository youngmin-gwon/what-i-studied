...
- **시장 확장**: 한국어만 지원하면 5 천만 시장이지만, 영어와 스페인어만 추가해도 10 억 시장이 됩니다. 처음부터 다국어 구조(`NSLocalizedString`)를 잡지 않으면 나중에 모든 코드를 뜯어고쳐야 합니다.
...
QA 팀이나 CI/CD 파이프라인에서 UI 테스트를 돌릴 때, 버튼을 찾는 가장 안전한 방법은 접근성 ID 를 쓰는 것입니다.
...
>상세 비교는 [**android-accessibility-compose**](../../android/02_app_framework/jetpack-compose/layout-and-ui/compose-ui/semantics-tree-makes-ui-meaning-visible-to-accessibility-and-tests.md) 를 참고하세요.
