...
Core Data 를 쓴다면 코드 한 줄 없이 iCloud 동기화가 됩니다.

- **장점**: 로컬 DB(SQLite)와 클라우드 미러링을 OS 가 알아서 해줍니다. 오프라인 변경 사항도 자동으로 큐에 쌓였다가 연결되면 나갑니다.
- **단점**: "내 서버"가 아닙니다. 안드로이드나 웹이랑 공유하기 어렵습니다(CloudKit JS 가 있긴 하지만).
...
- 클라이언트: "나 100 번 버전까지 있어. 그 뒤에 일어난 일만 줘." (`since_version=100`)
- 서버: 101 번(추가), 102 번(삭제) 변경 로그만 리턴.

#### 3. Conflict Resolution (충돌 해결)
- **Last Writer Wins (LWW)**: 가장 최근 타임스탬프가 이깁니다. (구현 쉽지만 데이터 유실 위험)
- **Manual Merge**: 사용자에게 "두 버전이 충돌했습니다" 팝업을 띄웁니다. (Git 처럼)

---

### 📷 Media Sync (대용량 파일)

사진이나 동영상은 JSON 과 같이 보내면 안 됩니다.

1. **2-Phase Upload**:
   - 1 단계: 서버에 "사진 올릴게" 요청 -> 서버가 "여기다 올려(Presigned URL)" 응답.
