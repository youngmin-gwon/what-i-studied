...
- **Cost**: 출시 후 보안 패치를 하는 비용은 개발 단계에서 수정하는 비용의 100 배입니다(긴급 배포, 사용자 공지, 법적 대응 등).
...
- [ ] **API Keys**: AWS Access Key 나 Firebase Key 가 소스코드에 하드코딩되어 있지 않나요? 환경 변수나 난독화, 혹은 서버 사이드 프록시를 사용하세요.

---

### 🆔 인증과 권한 (Auth & Authz)

- [ ] **Biometrics Fallback**: FaceID 가 실패하거나 잠겼을 때, 비밀번호 입력으로 넘어가는 흐름(Fallback)이 구현되어 있나요?
