...
tags: [apple, apple/security, sandbox, mac]
---
...
Apple의 보안 모델은 **강제적 접근 제어(Mandatory Access Control, MAC)**를 기반으로 하며, 앱이 시스템 리소스나 사용자 데이터에 접근하는 것을 커널 수준에서 차단하는 **App Sandbox**가 핵심입니다.
...
3. **Hardened Runtime (macOS)**: 
    ...
    - `Library Validation`: Apple이나 개발자 본인이 서명한 라이브러리만 로드하도록 지장합니다.
...
- 파일 URL을 `BookmarkData`로 변환하여 영구 보존합니다.
