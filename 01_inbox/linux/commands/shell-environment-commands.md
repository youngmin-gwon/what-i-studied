...
| **C Shell (csh/tcsh)** | C언어와 유사한 문법. 히스토리, 작업 제어 중심. |
...
> [!NOTE]
> **명령어 우선순위**: 1순위: **Alias** > 2순위: **내부 명령어(Builtin)** > 3순위: **함수(Function)** > 4순위: **외부 명령어($PATH 경로)**
...
| **`$PS1`** / **`$PS2`** | 셸 프롬프트 모양 정의 | 1차(기본) 및 2차(미완성 시) 프롬프트 |
...
| **`\t`** / **`\T`** | 24시 / 12시 형태의 시간 | `23:59:59`, `11:59:59` |
| **`\@`** | 12시 형태 시간 + AM/PM | `11:59 PM` |
| **`\s`** | 현재 사용 중인 셸 이름 | `bash` |
| **`\!`** | 히스토리 번호 | `501` |
| **`\$`** | 사용자 구분 (root 면 `#`, 아니면 `$`) | `$` |
| **`\n`** | 줄바꿈 (New line) | 프롬프트를 2줄로 구성 시 사용 |
