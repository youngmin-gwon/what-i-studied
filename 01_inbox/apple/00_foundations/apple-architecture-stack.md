...
Apple 운영체제의 기반인 Darwin 과 XNU 커널의 아키텍처 심층 분석. 용어 정의는 [apple-glossary](apple-glossary.md) 참고.
...
- **Tasks & Threads**: 프로세스(Task)와 실행 단위(Thread) 관리. BSD 의 프로세스 모델은 Mach Task 위에서 구현됩니다.
...
#### 2. BSD (Berkeley Software Distribution Layer)
...
3. **Dyld**: 동적 링커(`dyld`)가 프로세스 메모리에 로드됨.
   ...
   - **Load Dylibs**: 앱이 사용하는 dylib 들을 로드하고 심볼 바인딩(Rebase/Bind).
...
- [apple-memory-management](../01_language_concurrency/apple-memory-management.md) - 메모리 관리 상세: Jetsam 과 Swap
