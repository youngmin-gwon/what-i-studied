...
aliases: []
date modified: 2026-04-05 12:18:06 +09:00
...
- **크로스 플랫폼 설계**: iOS 앱을 macOS 나 visionOS 로 확장할 때, 무엇이 공통점이고 차이점인지 알면 코드 재사용률을 높일 수 있습니다.
...
4. **버전 체계의 변화 (iOS 26+)**:
   - 2025 년, Apple 은 모든 OS 버전을 출시 연도와 맞추는 **대점프(Version Jump)**를 단행했습니다. 이에 따라 iOS 19 다음 버전은 **iOS 26**이 되었으며, 모든 플랫폼(macOS, watchOS 등)이 동일한 메인 버전 번호를 공유합니다.
...
1. **Entrypoint**: `@main` 어트리뷰트가 붙은 타입(SwiftUI `App` 또는 `@UIApplicationMain`)이 진입점이 되며, Swift 런타임을 초기화하고 **Run Loop**를 돌립니다. (내부적으로 C 의 `main` 함수가 자동 생성됩니다.)
...
1. **기반 다지기**:
   - [apple-architecture-stack](apple-architecture-stack.md): XNU 커널과 Darwin 의 이해
