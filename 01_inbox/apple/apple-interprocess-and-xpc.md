...
tags: [apple, ipc, xpc]
aliases: []
date modified: 2025-12-16 16:15:31 +09:00
...
### Extension 과 XPC
- 공유 시트, 위젯, 인텐트, Safari 확장 등은 모두 .appex 이며 XPC 로 주 앱과 통신한다.
- 데이터는 Encodable/Decodable 또는 NSCoding 으로 직렬화해 전달.

### 보안
- 코드 서명/[[apple-glossary#Entitlement|Entitlement]] 로 "누가 누구에게 연결 가능한지"가 정해진다.
- Audit token/UID/PID 가 함께 전달되어 권한을 확인한다.
...
- 네트워크 IPC 는 URLSession/WebSocket/Bonjour 등으로 해결한다.

### 디버깅
- Console.app 에서 XPC 로그 필터링.
- `launchctl print system/user` 로 등록된 서비스 확인.
- Instruments System Trace 로 프로세스/스레드/XPC 메시지 타임라인 보기.
