...
2. **Bash에서 배열 길이**를 구하는 표현은?  
   **A:** `${#arr[@]}`. 문자열 길이는 `${#str}`.
3. **`[$a == $b](../../$a == $b.md)`와 `[ "$a" = "$b" ]`의 차이**는?  
   **A:** `[](../.md)`는 글로브 확장/단어분리 없음, `==`는 패턴 매칭. test는 단순 문자열 비교이며 인용 필수.
...
18. **문제**: `[.+](../../$path =~ ^/home/.+.md)` 정규식 매칭이 실패한다. `$path`에 `/`가 포함되어서일까?  
    **해설**: 인용하면 리터럴로 처리되어 정규식 동작. `[.+](../../$path =~ ^/home/.+.md)`에서 `$path`를 인용하지 않는다. 또는 정규식에 `^/home/.+` 올바른지 확인.
...
25. **문제**: `[-s $file](../../-s $file.md)`와 `[ -s "$file" ]`의 차이는?  
    **해설**: 거의 없음. 단, `[](../.md)`는 인용 필요 없고, 글로브/단어분리 억제.
...
### G3. POSIX 호환 스크립트 변환
- **문제**: Bash 배열/`[](../.md)`를 사용한 스크립트를 `/bin/sh` 호환으로 고쳐라.
- **풀이 포인트**: 배열 대신 줄 단위 파일 또는 스페이스 구분 문자열 사용, `set --`/`for`로 순회. `[...](../../....md)` → `[ ... ]` + 인용. `function` 키워드 제거, `$(())`는 POSIX 지원되나 `(( ))`는 사용 자제. process substitution 제거 후 명명 파이프/임시 파일로 대체.
...
## Part H. 짧은 OX/선택 문제
- `[-v VAR](../../-v VAR.md)`는 변수가 설정되었는지 확인한다. (O)
...
- `[foo > bar](../../foo > bar.md)`는 문자열 사전순 비교다. (O)
...
6. `[$PS1](../../$PS1.md)` 체크가 의미하는 바를 설명하고, 비인터랙티브 셸에서 실행될 코드를 제한하는 방법을 적어라.  
