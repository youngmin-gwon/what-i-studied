...
date modified: 2026-08-05 11:42:25 +09:00
date created: 2026-08-05 11:42:00 +09:00
---

## 익명 파이프와 FIFO 는 커널 링 버퍼 기반의 단방향 바이트 스트림 통신이다

>**핵심 명제**: 익명 파이프(Anonymous Pipe)와 FIFO(Named Pipe)는 커널 메모리 내 링 버퍼(Ring Buffer)를 메개로 한 단방향 바이트 스트림 IPC 채널이다. 부모 - 자식 프로세스 간 혈연관계 제약 유무에 따라 구분되며, `PIPE_BUF` 크기 이내의 쓰기 작업에 대해 atomic 단방향 데이터를 보장한다.
...
1. **커널 링 버퍼 (Kernel Ring Buffer)**
   - `pipe()` 시스템 콜 실행 시 커널은 메모리 상에 링 버퍼(기본 Linux 기준 64KB, `fcntl(F_SETPIPE_SZ)` 로 변경 가능)를 할당하고 읽기 전용(`pipefd[0]`)과 쓰기 전용(`pipefd[1]`) 파일 디스크립터(FD) 쌍을 반환한다.
2. **Atomic Write 와 `PIPE_BUF`**
   - Linux 환경에서 `PIPE_BUF` 크기(기본 4096 bytes / 4KB) 이내의 데이터를 `write()` 할 경우 커널은 인터리빙(Interleaving, 타 프로세스 데이터와 섞임) 없이 단일 Atomic 작업으로 전송을 보장한다.
...
3. **익명 파이프 vs FIFO (Named Pipe) 차이**
   ...
   - **FIFO (Named Pipe)**: `mkfifo()` 를 통해 파일시스템 디렉터리에 VFS 노드(Special File)로 노출된다. 혈연관계가 없는 완전히 독립적인 프로세스들이 파일 경로명(`open()`)을 통해 파이프 버퍼를 공유한다.
