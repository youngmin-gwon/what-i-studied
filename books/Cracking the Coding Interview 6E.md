...
'실제 문제'에 대비해야 하고, '실제 문제'를 놓고 연습하고, 그 패턴을 익혀야 함. 이는 단순히 기존의 문제를 암기하는 것이 아니라, 새로운 알고리즘을 개발할줄 아는 것을 말한다
...
3. 기술적 지식 및 컴퓨터 과학 기본: 기본기 출중한가
4. 경험: 기술적 결정 해본적 있는가, 도전적인 프로젝트를 해본적 있는가, 진취성, 결단성, 그 외 다른 중요요소를 내보인 적이 있는가
5. 문화와 얼마나 맞는지/ 의사소통 능력: 지원자 성격과 가치가 회사에 잘 맞는가

알고리즘 문제를 풀 때는 처음 세 가지 능력이 중요함.

문제풀이 능력은 꽤 가치 있는 능력이므로, 개발하는 능력이 뛰어날 가능성이 크다는 것을 반증하는 것임.
...
준비 과정 중하나로, 두세 개의 프로젝트를 집중적으로 깊이 있게 알고 있어야 한다. 다음의 기준에 꼭 맞는 프로젝트를 선택할 수 있다면 더할 나위 없다
...
- 최적화: ㅇ모든 원소를 해시테이블에 넣고 x+k 혹은 x-k가 배열에 존재하는지 확인
